...
current_track_free(gpointer p)
{
    ...
    if (t->artists)
        g_ptr_array_unref(t->artists);
    g_free(t);
}

static void
track_artist_free_gp(gpointer p)
{
...
static void
album_tracks_free(gpointer p)
{
    album_tracks_t *b = p;
    if (!b)
        return;
    if (b->by_path)
        g_hash_table_destroy(b->by_path);
    g_free(b);
}
...
static char *
json_array_from_ids(const int64_t *ids, size_t n)
{
...
static void
snapshot_init(reconcile_snapshot_t *s)
{
    s->albums = g_hash_table_new_full(g_int64_hash, g_int64_equal, g_free, current_album_free);
    s->tracks_by_album
        = g_hash_table_new_full(g_int64_hash, g_int64_equal, g_free, album_tracks_free);
    s->tracks_by_id
        = g_hash_table_new_full(g_int64_hash, g_int64_equal, g_free, current_track_free);
}

static void
snapshot_free(reconcile_snapshot_t *s)
{
    if (s->albums)
        g_hash_table_destroy(s->albums);
    if (s->tracks_by_album)
        g_hash_table_destroy(s->tracks_by_album);
    if (s->tracks_by_id)
        g_hash_table_destroy(s->tracks_by_id);
...
static void
snapshot_load(quadrature_db_t *db, const int64_t *album_ids, size_t n, reconcile_snapshot_t *snap)
{
    snapshot_init(snap);
    if (n == 0)
        return;
    ...
    while (sqlite3_step(s) == SQLITE_ROW) {
        ...
        if (!bucket) {
            ...
            bucket->by_path
                = g_hash_table_new(g_str_hash, g_str_equal); /* keys owned by current_track_t */
...
static bool
str_equal_nullsafe(const char *a, const char *b)
{
    if (a == b)
        return true;
    if (!a)
...
static bool
artist_credits_equal(const GPtrArray *cur, const desired_track_artist_t *des, size_t n)
{
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (c->artist_id != des[i].artist_id)
            return false;
        if (!str_equal_nullsafe(c->join_phrase, des[i].join_phrase ? des[i].join_phrase : ""))
            return false;
        if (c->position != des[i].position)
            return false;
    }
    return true;
}
...
static char *
build_artist_display(const desired_track_artist_t *a, size_t n)
{
    if (n == 0)
        return g_strdup("");
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        if (jp && jp[0] && i + 1 < n)
            g_string_append(s, jp);
    }
    return g_string_free(s, FALSE);
}

static char *
merge_genre_strings(const char *existing, const char *incoming)
{
    if (!existing || !existing[0])
        return incoming ? g_strdup(incoming) : NULL;
...
static int
album_update_canonical(quadrature_db_t *db,
                       ...
                       bool *fts_dirty)
{
    ...
    int64_t artist_id
        = (d->present_fields & DESIRED_ALBUM_ARTIST_ID) ? d->artist_id : cur->artist_id;
    bool is_comp
        = (d->present_fields & DESIRED_ALBUM_COMPILATION) ? d->is_compilation : cur->is_compilation;
    uint16_t year = (d->present_fields & DESIRED_ALBUM_YEAR) ? d->year : cur->year;
    const char *rid = (d->present_fields & DESIRED_ALBUM_MB_RELEASE_ID) ? d->musicbrainz_release_id
                                                                        : cur->mb_release_id;
    const char *rgid = (d->present_fields & DESIRED_ALBUM_MB_RELEASE_GROUP)
                           ? d->musicbrainz_release_group_id
                           : cur->mb_release_group_id;
    int mb_status = (d->present_fields & DESIRED_ALBUM_MB_STATUS) ? d->mb_status : cur->mb_status;
    int64_t mb_resolved = (d->present_fields & DESIRED_ALBUM_MB_RESOLVED_AT) ? d->mb_resolved_at
                                                                             : cur->mb_resolved_at;
    ...
    if (title_diff)
        changes++;
    bool artist_diff = artist_id != cur->artist_id;
    if (artist_diff)
        changes++;
    if (is_comp != cur->is_compilation)
        changes++;
    if (year != cur->year)
        changes++;
    if (!str_equal_nullsafe(rid, cur->mb_release_id))
        changes++;
    if (!str_equal_nullsafe(rgid, cur->mb_release_group_id))
        changes++;
    if (mb_status != cur->mb_status)
        changes++;
    if (mb_resolved != cur->mb_resolved_at)
        changes++;
...
static bool
track_update_canonical(quadrature_db_t *db,
                       ...
                       int *genre_changes)
{
    ...
    uint32_t duration_ms
        = (d->present_fields & DESIRED_TRACK_DURATION) ? d->duration_ms : cur->duration_ms;
    ...
    const char *artist_display
        = (d->present_fields & DESIRED_TRACK_ARTISTS) ? new_display : cur->artist_display;
    ...
    if (!title_diff && !tnum_diff && !disc_diff && !dur_diff && !year_diff && !mtime_diff
        && !genre_diff && !display_diff) {
        return false;
    }
    ...
    if (title_diff || display_diff)
        *fts_dirty = true;
    if (title_diff)
        (*title_changes)++;
    if (tnum_diff || disc_diff)
        (*position_changes)++;
    if (genre_diff)
        (*genre_changes)++;
    return true;
...
static int64_t
insert_track(quadrature_db_t *db, int64_t album_id, const desired_track_t *d)
{
    if (!d->path)
        return 0;
    ...
    const char *title
        = (d->present_fields & DESIRED_TRACK_TITLE) && d->title ? d->title : "Unknown";
    ...
    const char *genre
        = (d->present_fields & (DESIRED_TRACK_GENRE | DESIRED_TRACK_GENRE_MERGE)) ? d->genre : NULL;
    ...
    if (genre)
        sqlite3_bind_text(s, 9, genre, -1, SQLITE_STATIC);
    else
        sqlite3_bind_null(s, 9);
    if (display)
        sqlite3_bind_text(s, 10, display, -1, SQLITE_STATIC);
    else
...
static void
replace_track_artists(quadrature_db_t *db,
                      int64_t track_id,
                      const desired_track_artist_t *a,
                      size_t n)
{
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        sqlite3_bind_text(db->insert_track_artist,
                          4,
                          a[i].join_phrase ? a[i].join_phrase : "",
                          -1,
                          SQLITE_STATIC);
              ...
              reconcile_summary_t *summary_out)
{
    ...
    if (desired->source == RECONCILE_SOURCE_TAGS && policy->respect_user_edits
        ...
        d_local.present_fields &= ~(DESIRED_ALBUM_TITLE | DESIRED_ALBUM_ARTIST_ID
                                    | DESIRED_ALBUM_MB_RELEASE_ID | DESIRED_ALBUM_MB_RELEASE_GROUP
...
quadrature_result_t
db_reconcile_albums(quadrature_db_t *db,
                    ...
                    reconcile_summary_t *summaries_out)
{
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (!cur_album) {
            if (summaries_out)
                summaries_out[i] = (reconcile_summary_t){ 0 };
...
quadrature_result_t
db_delete_album(quadrature_db_t *db, int64_t album_id)
{
    if (!db || album_id <= 0)
        return QUADRATURE_ERROR_INVALID_PARAM;
    ...
    sqlite3_prepare_v2(db->db, "DELETE FROM tracks WHERE album_id = ?", -1, &del_tracks, NULL);
    sqlite3_bind_int64(del_tracks, 1, album_id);
    sqlite3_step(del_tracks);
    sqlite3_finalize(del_tracks);

    sqlite3_stmt *del_album = NULL;
    sqlite3_prepare_v2(db->db, "DELETE FROM albums WHERE id = ?", -1, &del_album, NULL);
...
quadrature_result_t
db_create_or_get_album_by_path(quadrature_db_t *db,
                               const char *path,
                               const char *title,
                               int64_t artist_id,
                               uint16_t year,
                               int64_t *album_id_out)
{
    ...
    *album_id_out = 0;
    ...
    if (artist_id > 0)
        ...
        sqlite3_bind_null(db->insert_folder_album, 2);
...
quadrature_result_t
db_reconcile_album_mb_status(quadrature_db_t *db,
                             int64_t album_id,
                             int mb_status,
                             int64_t mb_resolved_at)
{
