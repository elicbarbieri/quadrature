...
static const char* BIOS_SCHEMA_SQL =
...
static void apply_bios_pragmas(sqlite3* db) {
...
static void migrate_from_metadata_db(sqlite3* bios_db, const char* library_root) {
    ...
    if (rc != SQLITE_OK) {
        g_debug("db_bios: could not attach metadata DB for migration: %s",
                err ? err : "unknown");
        sqlite3_free(err);
        return;
    }
...
static sqlite3_stmt* bios_get_stmt(quadrature_bios_db_t* db,
...
quadrature_result_t db_bios_open(const char* library_root,
                                  quadrature_bios_db_t** out) {
...
quadrature_result_t db_bios_open_readonly(const char* library_root,
                                           quadrature_bios_db_t** out) {
    if (!library_root || !out) return QUADRATURE_ERROR_INVALID_PARAM;
...
void db_bios_close(quadrature_bios_db_t* db) {
    if (!db) return;
    if (db->stmt_upsert) { sqlite3_finalize(db->stmt_upsert); db->stmt_upsert = NULL; }
    sqlite3_close(db->db);
    g_free(db);
}
...
quadrature_result_t db_bios_upsert(quadrature_bios_db_t* db, const char* artist_mbid,
    ...
    if (!db || !artist_mbid || !bio_text) return QUADRATURE_ERROR_INVALID_PARAM;

    sqlite3_stmt* stmt = bios_get_stmt(db, &db->stmt_upsert,
...
quadrature_result_t db_bios_exists(quadrature_bios_db_t* db, const char* artist_mbid,
    ...
    *exists_out = false;
    ...
    sqlite3_stmt* stmt = NULL;
    int rc = sqlite3_prepare_v2(db->db,
        "SELECT 1 FROM artist_bios WHERE artist_mbid = ? LIMIT 1",
        -1, &stmt, NULL);
