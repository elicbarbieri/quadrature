...
quadrature_result_t
db_get_track(quadrature_db_t *db, int64_t id, db_track_t **out)
{
    if (!db || !out)
...
quadrature_result_t
db_get_album_by_id(quadrature_db_t *db, int64_t album_id, db_album_t **out)
{
    if (!db || !out)
        return QUADRATURE_ERROR_INVALID_PARAM;
    *out = NULL;
...
static void
fill_track_from_row(sqlite3_stmt *stmt, db_track_t *t, bool borrow)
{
...
quadrature_result_t
db_get_tracks_by_album(quadrature_db_t *db, int64_t album_id, db_track_t **out, size_t *count)
{
    if (!db || !out || !count)
        return QUADRATURE_ERROR_INVALID_PARAM;
    ...
    g_debug("db_get_tracks_by_album(%" G_GINT64_FORMAT "): %zu results in %.2f ms",
            album_id,
            i,
            elapsed / 1000.0);
...
quadrature_result_t
db_iter_all_artists(quadrature_db_t *db, db_artist_iter_cb cb, void *user_data)
{
    if (!db || !cb)
        return QUADRATURE_ERROR_INVALID_PARAM;
...
quadrature_result_t
db_iter_all_albums(quadrature_db_t *db, db_album_iter_cb cb, void *user_data)
{
    if (!db || !cb)
        return QUADRATURE_ERROR_INVALID_PARAM;
...
quadrature_result_t
db_iter_all_tracks(quadrature_db_t *db, db_track_iter_cb cb, void *user_data)
{
    if (!db || !cb)
        return QUADRATURE_ERROR_INVALID_PARAM;
...
quadrature_result_t
db_iter_all_track_artists(quadrature_db_t *db, db_track_artist_iter_cb cb, void *user_data)
{
    if (!db || !cb)
        return QUADRATURE_ERROR_INVALID_PARAM;
...
quadrature_result_t
db_get_max_ids(quadrature_db_t *db, int64_t *max_artist, int64_t *max_album, int64_t *max_track)
{
    if (!db)
        return QUADRATURE_ERROR_INVALID_PARAM;
    ...
    if (sqlite3_step(stmt) == SQLITE_ROW) {
        if (max_artist)
            *max_artist = sqlite3_column_int64(stmt, 0);
        if (max_album)
            *max_album = sqlite3_column_int64(stmt, 1);
        if (max_track)
            *max_track = sqlite3_column_int64(stmt, 2);
    } else {
        if (max_artist)
            *max_artist = 0;
        if (max_album)
...
void
db_albums_free(db_album_t *albums, size_t count)
{
    if (!albums)
        return;
...
void
db_tracks_free(db_track_t *tracks, size_t count)
{
    if (!tracks)
        return;
    for (size_t i = 0; i < count; i++) {
...
quadrature_result_t
db_resolve_track_positions_batch(quadrature_db_t *db,
                                 const db_track_position_t *positions,
                                 size_t count,
...
quadrature_result_t
db_get_artist_by_mbid(quadrature_db_t *db, const char *musicbrainz_id, int64_t *artist_id_out)
{
    ...
    *artist_id_out = 0;
    ...
    int rc = sqlite3_prepare_v2(
        db->db, "SELECT id FROM artists WHERE musicbrainz_id = ? LIMIT 1", -1, &stmt, NULL);
...
static const char *
get_artist_order_clause(db_sort_t sort)
{
    switch (sort) {
    case DB_SORT_NAME_DESC:
        return "ORDER BY a.name COLLATE NOCASE DESC";
    case DB_SORT_RECENT:
        return "ORDER BY a.id DESC";
    default:
        return "ORDER BY a.name COLLATE NOCASE ASC";
    }
}

static const char *
get_album_order_clause(db_sort_t sort)
{
    switch (sort) {
    case DB_SORT_NAME_ASC:
        return "ORDER BY al.title COLLATE NOCASE ASC";
    case DB_SORT_NAME_DESC:
        return "ORDER BY al.title COLLATE NOCASE DESC";
    case DB_SORT_YEAR_ASC:
        return "ORDER BY al.year ASC, al.title COLLATE NOCASE ASC";
    case DB_SORT_YEAR_DESC:
        return "ORDER BY al.year DESC, al.title COLLATE NOCASE ASC";
    case DB_SORT_ARTIST_ASC:
        return "ORDER BY ar.name COLLATE NOCASE ASC, al.year ASC, al.title COLLATE NOCASE ASC";
    case DB_SORT_ARTIST_DESC:
        return "ORDER BY ar.name COLLATE NOCASE DESC, al.year DESC, al.title COLLATE NOCASE DESC";
    case DB_SORT_ADDED_ASC:
        return "ORDER BY al.id ASC";
    case DB_SORT_ADDED_DESC:
        return "ORDER BY al.id DESC";
    case DB_SORT_RECENT:
        return "ORDER BY al.id DESC";
    default:
        return "ORDER BY ar.name COLLATE NOCASE ASC, al.year ASC, al.title COLLATE NOCASE ASC";
...
static void
sql_append_year_or(GString *sql, uint16_t year_mask, const char *col)
{
    gboolean first = TRUE;
    for (int i = 0; i < NUM_DECADE_BITS; i++) {
        ...
        g_string_append_printf(
            sql, "%s BETWEEN %u AND %u", col, DECADE_RANGES[i][0], DECADE_RANGES[i][1]);
        first = FALSE;
    }
}
...
static int
sql_bind_genres(sqlite3_stmt *stmt, int idx, const db_search_opts_t *opts)
{
    if (!opts)
        return idx;
...
quadrature_result_t
db_get_album_mtimes_page(
    quadrature_db_t *db, size_t offset, size_t limit, db_album_mtime_t **out, size_t *count_out)
{
    if (!db || !out || !count_out || limit == 0)
        return QUADRATURE_ERROR_INVALID_PARAM;
    ...
    *count_out = 0;
    ...
    sqlite3_prepare_v2(db->db,
                       "SELECT id, path, last_updated_at, last_updated_size, mb_status FROM albums "
                       "WHERE path != '' "
    ...
    while (sqlite3_step(stmt) == SQLITE_ROW && i < limit) {
        ...
        results[i].last_updated_at
            = sqlite3_column_type(stmt, 2) != SQLITE_NULL ? sqlite3_column_int64(stmt, 2) : 0;
        results[i].last_updated_size
            = sqlite3_column_type(stmt, 3) != SQLITE_NULL ? sqlite3_column_int64(stmt, 3) : 0;
...
void
db_free_album_mtimes(db_album_mtime_t *albums, size_t count)
{
    if (!albums)
        return;
    for (size_t i = 0; i < count; i++) {
...
quadrature_result_t
db_get_error_count(quadrature_db_t *db, const char *path_prefix, size_t *count)
{
    if (!db || !count)
        return QUADRATURE_ERROR_INVALID_PARAM;
...
quadrature_result_t
db_get_errors_page(quadrature_db_t *db,
                   const char *path_prefix,
                   size_t offset,
                   size_t limit,
                   db_indexer_error_t **out,
                   size_t *count)
{
    if (!db || !out || !count || limit == 0)
        return QUADRATURE_ERROR_INVALID_PARAM;
...
static char *
build_fts_query(const char *input)
{
    if (!input || !*input)
        return NULL;
               ...
               size_t *out_count)
{
    ...
    *out_count = 0;
    ...
    char *fts_query
        = (opts->search_text && opts->search_text[0]) ? build_fts_query(opts->search_text) : NULL;
    ...
    GString *sql_str
        = g_string_new("SELECT a.id FROM artists a WHERE ("
    ...
    if (has_search)
        g_string_append(sql_str,
                        " AND a.id IN (SELECT rowid FROM artists_fts WHERE artists_fts MATCH ?)");
...
static quadrature_result_t
get_album_ids(quadrature_db_t *db,
              const db_id_query_opts_t *opts,
              int64_t **out_ids,
              size_t *out_count)
{
    ...
    *out_count = 0;
    ...
    gboolean needs_artist_join
        = (opts->sort == DB_SORT_ARTIST_ASC || opts->sort == DB_SORT_ARTIST_DESC);
    ...
    if (has_search)
        g_string_append(sql_str,
                        " AND al.id IN (SELECT rowid FROM albums_fts WHERE albums_fts MATCH ?)");
    if (has_year) {
...
static quadrature_result_t
get_track_ids(quadrature_db_t *db,
              const db_id_query_opts_t *opts,
              int64_t **out_ids,
              size_t *out_count)
{
    const char *query = opts ? opts->search_text : NULL;
    if (!db || !query || !out_ids || !out_count)
        return QUADRATURE_ERROR_INVALID_PARAM;
    ...
    *out_count = 0;
    ...
    GString *sql_str = g_string_new("SELECT t.id FROM tracks_fts f"
...
quadrature_result_t
db_get_entity_ids_filtered(quadrature_db_t *db,
                           db_entity_t entity,
                           const db_id_query_opts_t *opts,
                           int64_t **out_ids,
                           size_t *out_count)
{
    switch (entity) {
    case DB_ENTITY_ARTIST:
        return get_artist_ids(db, opts, out_ids, out_count);
    case DB_ENTITY_ALBUM:
        return get_album_ids(db, opts, out_ids, out_count);
    case DB_ENTITY_TRACK:
        return get_track_ids(db, opts, out_ids, out_count);
    }
    return QUADRATURE_ERROR_INVALID_PARAM;
}
...
quadrature_result_t
db_get_artists_with_mbid(quadrature_db_t *db, int64_t **artist_ids, char ***mbids, size_t *count)
{
    if (!db || !artist_ids || !mbids || !count)
        return QUADRATURE_ERROR_INVALID_PARAM;
    *artist_ids = NULL;
    *mbids = NULL;
    *count = 0;
    ...
    sqlite3_stmt *stmt;
    int rc = sqlite3_prepare_v2(
        db->db, "SELECT COUNT(*) FROM artists WHERE musicbrainz_id IS NOT NULL", -1, &stmt, NULL);
    if (rc != SQLITE_OK) {
        db_unlock(db);
        return QUADRATURE_ERROR_INTERNAL;
    }
...
quadrature_result_t
db_get_albums_with_release_group_id(quadrature_db_t *db,
                                    int64_t **album_ids,
                                    char ***release_group_ids,
                                    char ***artist_mbids,
                                    size_t *count)
{
...
quadrature_result_t
db_get_entity_count(quadrature_db_t *db, db_entity_t entity, size_t *count)
{
    if (!db || !count)
        return QUADRATURE_ERROR_INVALID_PARAM;
    *count = 0;
...
quadrature_result_t
db_get_last_indexed_time(quadrature_db_t *db, int64_t *unix_time)
{
    if (!db || !unix_time)
        return QUADRATURE_ERROR_INVALID_PARAM;
    *unix_time = 0;
    ...
    sqlite3_stmt *stmt;
    int rc = sqlite3_prepare_v2(
        db->db, "SELECT COALESCE(MAX(last_updated_at), 0) FROM albums", -1, &stmt, NULL);
    if (rc != SQLITE_OK) {
        db_unlock(db);
        return QUADRATURE_ERROR_INTERNAL;
    }
