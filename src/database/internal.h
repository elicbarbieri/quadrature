...
struct quadrature_db {
    ...
    sqlite3_stmt *
        sync_album_tracks_fts; /* Bulk INSERT OR REPLACE INTO tracks_fts for all tracks in an album */
    ...
    sqlite3_stmt
        *move_track_artists; /* UPDATE OR IGNORE track_artists SET artist_id=? WHERE artist_id=? */
    ...
    sqlite3_stmt
        *iter_all_albums; /* SELECT id, title, artist_id, year, path, mb_release_id FROM albums */
    sqlite3_stmt *iter_all_tracks; /* SELECT id, title, path, ... FROM tracks (no JOINs) */
    sqlite3_stmt
        *iter_all_track_artists; /* SELECT track_id, artist_id, join_phrase, position (no JOIN) */
    ...
    sqlite3_stmt
        *update_album_mb_status; /* UPDATE albums SET mb_status=?, mb_resolved_at=? WHERE id=? */
    sqlite3_stmt *reconcile_load_albums_batch; /* WHERE id IN (SELECT value FROM json_each(?)) */
    sqlite3_stmt
        *reconcile_load_tracks_batch; /* WHERE album_id IN (SELECT value FROM json_each(?)) */
    sqlite3_stmt *
        reconcile_load_track_artists_batch; /* WHERE track_id IN (SELECT value FROM json_each(?)) */
    ...
    sqlite3_stmt *
        select_artist_by_name_and_mbid; /* SELECT id FROM artists WHERE name=? COLLATE NOCASE AND musicbrainz_id=? */
