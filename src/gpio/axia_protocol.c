 ...
 */
bool
axia_protocol_parse_gpi(const char *line, int *out_channel, int *out_pin, int *out_state)
{
    if (!line || !out_channel || !out_pin || !out_state)
        return false;
 ...
 */
int
axia_protocol_format_gpo(char *buffer, int channel, int pin, int state)
{
    if (!buffer || channel < 1 || channel > 4 || pin < 1 || pin > 5)
        return 0;
...
int
axia_protocol_format_login(char *buffer, const char *password)
{
    if (!buffer || !password)
        return 0;
