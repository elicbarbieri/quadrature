...
quadrature_result_t axia_gpio_create(const char *address, int channel_id,
    ...
    *out = gpio;
    LOG_DEBUG("Created GPIO handler for channel %d (%s:%u)", channel_id + 1, gpio->console_ip, gpio->port);
...
quadrature_result_t axia_gpio_start(axia_gpio_t *gpio) {
    if (!gpio) return QUADRATURE_ERROR_INVALID_PARAM;
    ...
    if (result != 0) {
        LOG_ERROR("Channel %d: Failed to create listener thread: %s", 
                  gpio->channel_id + 1, strerror(result));
...
quadrature_result_t axia_gpio_set(axia_gpio_t *gpio, axia_pin_t pin, axia_state_t state) {
    ...
    if (sent < 0) {
        LOG_ERROR("Channel %d: Failed to send GPO command: %s", 
                  gpio->channel_id + 1, strerror(errno));
        atomic_store(&gpio->connected, false);
        return QUADRATURE_ERROR_INTERNAL;
    }
    
    LOG_INFO("Channel %d: Sent GPO pin=%d state=%s", 
             gpio->channel_id + 1, pin, state == AXIA_STATE_HIGH ? "HIGH" : "LOW");
...
static bool connect_to_console(axia_gpio_t *gpio) {
    ...
    if (sockfd < 0) {
        LOG_ERROR("Channel %d: Failed to create socket: %s", 
                  gpio->channel_id + 1, strerror(errno));
        return false;
    }
    ...
    if (inet_pton(AF_INET, gpio->console_ip, &addr.sin_addr) <= 0) {
        LOG_ERROR("Channel %d: Invalid IP address: %s", 
                  gpio->channel_id + 1, gpio->console_ip);
        close(sockfd);
        return false;
    }
    ...
    gpio->sockfd = sockfd;
    LOG_DEBUG("Channel %d: TCP connected to %s:%u", 
              gpio->channel_id + 1, gpio->console_ip, gpio->port);
...
static bool send_subscription(axia_gpio_t *gpio) {
    ...
    if (send(gpio->sockfd, cmd, len, MSG_NOSIGNAL) < 0) {
        LOG_ERROR("Channel %d: Failed to send ADD GPI: %s", 
                  gpio->channel_id + 1, strerror(errno));
        return false;
    }
    ...
    if (send(gpio->sockfd, cmd, len, MSG_NOSIGNAL) < 0) {
        LOG_ERROR("Channel %d: Failed to send ADD GPO: %s", 
                  gpio->channel_id + 1, strerror(errno));
        return false;
    }
...
static gboolean invoke_gpio_callback_on_main_thread(gpointer data) {
    gpio_callback_data_t *cb_data = (gpio_callback_data_t *)data;
    
    g_mutex_lock(&cb_data->gpio->callback_mutex);
    if (cb_data->gpio->callback) {
        cb_data->gpio->callback(cb_data->channel_id, cb_data->pin, cb_data->state, 
                               cb_data->gpio->callback_user_data);
    }
...
static gboolean invoke_status_callback_on_main_thread(gpointer data) {
    status_callback_data_t *cb_data = (status_callback_data_t *)data;
    
    g_mutex_lock(&cb_data->gpio->callback_mutex);
    if (cb_data->gpio->status_callback) {
        cb_data->gpio->status_callback(cb_data->channel_id, cb_data->connected, 
                                       cb_data->gpio->status_callback_user_data);
    }
    g_mutex_unlock(&cb_data->gpio->callback_mutex);
...
static void exponential_backoff_sleep(axia_gpio_t *gpio) {
    if (!atomic_load(&gpio->running)) return;
