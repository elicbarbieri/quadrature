...
static library_credit_info_t *credit_info_new(const char *artist_name,
...
static void credit_info_free(gpointer data) {
...
static void credit_info_add_role(library_credit_info_t *info, const char *role) {
    if (!role || !role[0]) return;
    for (guint i = 0; i < info->roles->len; i++) {
        if (g_strcmp0(g_ptr_array_index(info->roles, i), role) == 0) return;
    }
    g_ptr_array_add(info->roles, g_strdup(role));
}

static void credit_info_merge(library_credit_info_t *dst,
                              const library_credit_info_t *src) {
...
static void append_meta_artist(GPtrArray *meta_artists, GHashTable *seen_mbids,
                                const db_meta_artist_search_result_t *a) {
    if (!a->artist_mbid) return;
    if (g_hash_table_contains(seen_mbids, a->artist_mbid)) return;
...
static void gather_library(library_cache_t *cache, int bitmap_index,
                            const char *credit_text, const char *role_gid,
    ...
    if (!dbs.meta || !dbs.db) return;

    db_meta_artist_search_result_t *artists = NULL;
    size_t artist_count = 0;
    if (db_meta_search_artists(dbs.meta, credit_text, 50, &artists, &artist_count)
            != QUADRATURE_OK)
        return;
    if (artist_count == 0) {
    ...
    for (size_t ai = 0; ai < artist_count; ai++) {
        ...
        if (db_meta_get_credits_by_artist(dbs.meta, artists[ai].artist_mbid,
                                           role_gid, &credits, &credit_count)
                != QUADRATURE_OK || credit_count == 0) {
        ...
        for (size_t ci = 0; ci < credit_count; ci++) {
            ...
            if (!info) {
                ...
                int64_t *k = g_new(int64_t, 1); *k = global_id;
...
static char *track_dedup_key(const library_track_info_t *track,
...
static char *album_dedup_key(const library_album_info_t *album) {
    if (album && album->musicbrainz_release_group_id
            && album->musicbrainz_release_group_id[0]) {
...
library_credit_search_result_t *library_credit_search(library_cache_t *cache,
    ...
    r->credit_info  = g_hash_table_new_full(g_int64_hash, g_int64_equal,
                                             g_free, credit_info_free);
    r->meta_artists = g_ptr_array_new_with_free_func(g_free);

    if (!credit_text || !credit_text[0]) return r;

    /* Phase 1: raw hits keyed by global track_id. */
    GHashTable *raw_hits = g_hash_table_new_full(g_int64_hash, g_int64_equal,
                                                  g_free, credit_info_free);
    GHashTable *seen_meta_mbids = g_hash_table_new_full(g_str_hash, g_str_equal,
                                                         g_free, NULL);
    ...
    for (int li = 0; li < lib_count; li++) {
        ...
        if (!(library_mask & (1u << bi))) continue;
        if (!library_cache_get_available(cache, bi)) continue;
        gather_library(cache, bi, credit_text, role_gid,
                       raw_hits, r->meta_artists, seen_meta_mbids);
    }
    ...
    GHashTable *surviving_by_key = g_hash_table_new_full(g_str_hash, g_str_equal,
                                                          g_free, NULL);
    GHashTable *seen_album_keys = g_hash_table_new_full(g_str_hash, g_str_equal,
                                                         g_free, NULL);
    ...
    while (g_hash_table_iter_next(&iter, &k, &v)) {
        ...
        int64_t *survivor_id =
            (int64_t *)g_hash_table_lookup(surviving_by_key, key);

        if (survivor_id) {
            /* Dup — merge roles into survivor. */
            library_credit_info_t *survivor_info =
                g_hash_table_lookup(r->credit_info, survivor_id);
            if (survivor_info) credit_info_merge(survivor_info, incoming);
            g_free(key);
            continue;
        }
        ...
        int64_t *tid = g_new(int64_t, 1); *tid = global_id;
...
void library_credit_search_result_free(library_credit_search_result_t *r) {
    if (!r) return;
    if (r->track_ids)    g_array_unref(r->track_ids);
    if (r->album_ids)    g_array_unref(r->album_ids);
    if (r->credit_info)  g_hash_table_unref(r->credit_info);
