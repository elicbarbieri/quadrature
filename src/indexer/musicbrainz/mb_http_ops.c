...
static GBytes *
http_send_once(SoupSession *session,
               const char *method,
               const char *url,
...
http_send(SoupSession *session,
          ...
          guint *out_status)
{
    GBytes *body = http_send_once(session, method, url, content_type, req_body, out_status);
    const gboolean transient = (*out_status == 0) || (*out_status == 429) || (*out_status >= 500);
    if (transient) {
        if (body) {
            g_bytes_unref(body);
            body = NULL;
        ...
        g_usleep(1500 * 1000);
        body = http_send_once(session, method, url, content_type, req_body, out_status);
    }
    return body;
}
...
status_to_result(guint status)
{
    if (status >= 200 && status < 300)
        return QUADRATURE_OK;
    if (status == 404)
        return QUADRATURE_OK; /* not found = empty result */
...
static char *
dup_str(JsonObject *obj, const char *key)
{
    if (!obj || !json_object_has_member(obj, key))
        return NULL;
    if (json_object_get_null_member(obj, key))
        return NULL;
...
static int
get_int(JsonObject *obj, const char *key, int defv)
{
    if (!obj || !json_object_has_member(obj, key))
        return defv;
    if (json_object_get_null_member(obj, key))
...
static void
parse_recording_relations(const char *rec_mbid, JsonArray *relations, GPtrArray *arr)
{
    if (!relations)
        return;
    ...
    for (guint i = 0; i < n; i++) {
        ...
        JsonObject *artist = json_object_has_member(r, "artist")
                                 ? json_object_get_object_member(r, "artist")
                                 : NULL;
        if (!artist)
            continue; /* not an artist-rel */
        ...
        row->link_type_desc
            = NULL; /* ws/2 doesn't include this; PG has it from link_type.description */
...
static void
parse_artist_credits(JsonArray *credits, mb_artist_t **out_artists, size_t *out_count)
{
    if (!credits) {
        *out_artists = NULL;
        *out_count = 0;
...
static mb_release_t *
parse_release_with_links(JsonObject *root, GPtrArray *links_out)
{
    if (!root)
        return NULL;
    ...
    if (json_object_has_member(root, "artist-credit")) {
        parse_artist_credits(
            json_object_get_array_member(root, "artist-credit"), &r->artists, &r->artist_count);
    }
        ...
        for (guint mi = 0; mi < json_array_get_length(media); mi++) {
            ...
            for (guint ti = 0; ti < json_array_get_length(tracks); ti++) {
                ...
                if (recording) {
                    ...
                    if (links_out && rec.id && json_object_has_member(recording, "relations")) {
                        parse_recording_relations(
                            rec.id,
    ...
    mb_conn_t *conn, const char **ids, size_t n, GHashTable **out_releases, GHashTable **out_links)
{
    ...
    *out_releases = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, batch_release_free);
    *out_links
...
mb_http_isrc_lookup(mb_conn_t *conn, const char **isrcs, size_t n, mb_acoustid_response_t *out)
{
    ...
    for (size_t i = 0; i < n; i++) {
        if (!isrcs[i] || !isrcs[i][0])
            continue;
        mb_http_rate_limit_mb();

        char *url
            = g_strdup_printf("%s/isrc/%s?inc=releases+release-groups&fmt=json", base, isrcs[i]);
        guint status = 0;
        GBytes *body = http_send(session, "GET", url, NULL, NULL, &status);
        g_free(url);
        ...
        if (json_object_has_member(root, "recordings")) {
            ...
            for (guint ri = 0; ri < json_array_get_length(recs); ri++) {
                ...
                JsonArray *rels = json_object_has_member(rec, "releases")
                                      ? json_object_get_array_member(rec, "releases")
                                      : NULL;
                if (rels) {
...
quadrature_result_t
mb_http_fingerprint_lookup(mb_conn_t *conn, const mb_fingerprint_t *fp, mb_acoustid_response_t *out)
{
    g_assert(conn != NULL);
    g_assert(fp != NULL);
    g_assert(out != NULL);
    ...
    char *form = soup_form_encode("client",
                                  api_key,
                                  "format",
                                  "json",
                                  "meta",
    ...
    guint status = 0;
    GBytes *body
        = http_send(session, "POST", url, "application/x-www-form-urlencoded", form_bytes, &status);
    ...
    if (g_strcmp0(status_str, "ok") == 0 && json_object_has_member(root, "results")) {
        ...
        for (guint i = 0; i < json_array_get_length(results); i++) {
            ...
            if (!json_object_has_member(res_obj, "recordings"))
                continue;
...
static char *
strip_edition_suffix(const char *s)
{
    if (!s)
        return NULL;
    char *dup = g_strdup(s);
...
static double
score_candidate(JsonObject *rel,
                const char *q_album_raw,
                const char *q_artist,
                ...
                double solr_score)
{
    ...
    double title_s = 0.0;
    if (g_strcmp0(title, q_album_raw) == 0)
        title_s = 1.0;
    else if (ci_equal(title, q_album_raw))
        title_s = 0.7;
    else if (ci_equal(title, q_album_clean))
        title_s = 0.5;
    ...
    if (json_object_has_member(rel, "artist-credit")) {
        ...
        for (guint i = 0; i < json_array_get_length(ac); i++) {
            ...
            if (g_strcmp0(an, q_artist) == 0) {
                artist_s = 1.0;
                break;
            }
            if (ci_equal(an, q_artist))
                artist_s = MAX(artist_s, 0.7);
        }
    }
    ...
    if (candidate_tracks == 0)
        tracks_s = 0.3;
    else if ((int)local_track_count == candidate_tracks)
        tracks_s = 1.0;
    else if ((int)local_track_count < candidate_tracks)
        tracks_s = 0.3;
    else
...
static char *
http_escape_lucene(const char *input)
{
    static gsize once = 0;
...
quadrature_result_t
mb_http_solr_search(mb_conn_t *conn,
                    ...
                    char **out_release_id)
{
    ...
    (void)
        local_total_duration_ms; /* not validatable here — public search response lacks per-track lengths */
    *out_release_id = NULL;
    ...
    char *query = g_strdup_printf(
        "release:(%s) artist:(%s) tracks:%zu", esc_album, esc_artist, local_track_count);
    g_free(esc_album);
    g_free(esc_artist);
    char *enc_query = g_uri_escape_string(query, NULL, FALSE);
    g_free(query);

    char *url = g_strdup_printf("%s/release/?query=%s&limit=25&fmt=json", base, enc_query);
    g_free(enc_query);
