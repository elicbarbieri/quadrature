...
void
mb_http_rate_limit_mb(void)
{
    rate_limiter_init_once();
    rate_limiter_wait(&g_mb_rl);
}
void
mb_http_rate_limit_acoustid(void)
{
    rate_limiter_init_once();
    rate_limiter_wait(&g_acoustid_rl);
}
...
http_pool_state_t *
mb_http_slot_pool(http_slot_t *s)
{
    return s->pool;
}
void *
mb_http_slot_session(http_slot_t *s)
{
    return s->session;
}
const char *
mb_http_pool_mb_base(const http_pool_state_t *p)
{
    return p->mb_base_url;
}
const char *
mb_http_pool_acoustid_base(const http_pool_state_t *p)
{
    return p->acoustid_base_url;
}
const char *
mb_http_pool_acoustid_key(const http_pool_state_t *p)
{
    return p->acoustid_api_key;
}
...
http_pool_create(const mb_backend_config_t *cfg, size_t slot_count, mb_pool_t **out)
{
    g_assert(cfg != NULL);
    g_assert(out != NULL);
    ...
    state->mb_base_url
        = g_strdup((cfg->mb_base_url && cfg->mb_base_url[0]) ? cfg->mb_base_url
                                                             : "https://musicbrainz.org/ws/2");
    state->acoustid_base_url = g_strdup((cfg->acoustid_base_url && cfg->acoustid_base_url[0])
                                            ? cfg->acoustid_base_url
...
static void
http_pool_destroy(mb_pool_t *pool)
{
    if (!pool)
        return;
...
static mb_conn_t *
http_pool_claim_slot(mb_pool_t *pool, int slot)
{
    g_assert(pool != NULL);
    http_pool_state_t *state = (http_pool_state_t *)pool;
...
static bool
http_conn_reset(mb_conn_t *conn)
{
    g_assert(conn != NULL);
...
static size_t
http_pool_count(const mb_pool_t *pool)
{
    g_assert(pool != NULL);
...
static int
http_pool_claim_round_robin(mb_pool_t *pool)
{
    g_assert(pool != NULL);
...
static const char *
http_name(const mb_pool_t *pool)
{
    (void)pool;
    return "http";
}
...
quadrature_result_t
mb_backend_http_factory(const char *uri,
                        ...
                        mb_backend_t **out)
{
    ...
    if (slot_count > 1) {
        g_debug("mb_backend_http_factory: slot_count %zu clamped to 1 (rate limit)", slot_count);
        slot_count = 1;
    }
    ...
    mb_caps_t caps
        = MB_CAP_BATCH_FETCH | MB_CAP_ISRC_LOOKUP | MB_CAP_SOLR_SEARCH | MB_CAP_FINGERPRINT;
