...
static void
resolve_queue_item_free(resolve_queue_item_t *item)
{
    if (!item)
        return;
...
static void
resolver_update_progress(mb_resolver_t *ctx)
{
    if (!ctx->callback)
        return;
...
static void
resolver_set_phase(mb_resolver_t *ctx, mb_resolve_phase_t phase)
{
    g_mutex_lock(&ctx->progress_mutex);
 ...
 */
static void
mb_credits_to_track_artists(quadrature_db_t *db,
                            const mb_artist_t *credits,
                            size_t count,
                            db_track_artist_t **out,
                            size_t *out_count,
    ...
    for (size_t i = 0; i < count; i++) {
        ...
        if (artist_id == 0) {
            ...
            if (artist_cache && credits[i].id && credits[i].id[0] && artist_id > 0) {
                g_hash_table_insert(
                    artist_cache, g_strdup(credits[i].id), GSIZE_TO_POINTER((gsize)artist_id));
            }
        }
        ...
        (*out)[i].join_phrase
            = credits[i].joinphrase ? g_strdup(credits[i].joinphrase) : g_strdup("");
    }
}
...
static double
title_similarity(const char *a, const char *b)
{
    if (!a || !b)
        return 0.0;
...
static void
resolve_tags_free(resolve_tags_t *tags)
{
    if (!tags)
        return;
    g_free(tags->isrc);
 ...
 */
static void
read_resolve_tags(const char *audio_path, resolve_tags_t *out)
{
    ...
    while ((tag = av_dict_get(fmt->metadata, "", tag, AV_DICT_IGNORE_SUFFIX))) {
        if ((strcasecmp(tag->key, "isrc") == 0 || strcasecmp(tag->key, "TSRC") == 0)
            && !out->isrc) {
 ...
 */
static gint
cmp_count_desc(gconstpointer a, gconstpointer b, gpointer counts)
{
...
static void
dump_vote_distribution(int64_t album_id,
                       const char *stage,
                       ...
                       const char *picked_release)
{
    ...
    for (GList *r = rel_list; r; r = r->next) {
        ...
        g_debug("    release %s votes=%d%s", rel, votes, picked ? " [PICKED]" : "");
    }
 ...
 */
static void
tally_votes(const mb_acoustid_response_t *response,
            GHashTable *rg_counts,
            GHashTable *rg_best_release,
            GHashTable *release_counts,
            char **best_rg,
            int *best_rg_count)
{
    ...
    for (size_t j = 0; j < response->count; j++) {
        ...
        if (!g_hash_table_contains(seen_rgs, rg_id)) {
            ...
            g_hash_table_insert(rg_counts, g_strdup(rg_id), GINT_TO_POINTER(new_count));
        ...
        g_hash_table_insert(release_counts, g_strdup(rel_id), GINT_TO_POINTER(new_rcnt));

        const char *cur_best = g_hash_table_lookup(rg_best_release, rg_id);
        if (!cur_best
            || new_rcnt > GPOINTER_TO_INT(g_hash_table_lookup(release_counts, cur_best))) {
            g_hash_table_insert(rg_best_release, g_strdup(rg_id), g_strdup(rel_id));
        }
    }
...
static char *
find_release_by_fingerprint(mb_resolver_t *ctx,
                            int64_t album_id,
                            mb_backend_t *be,
                            mb_conn_t *conn,
                            ...
                            bool *service_error)
{
    ...
    if (!ctx->cancelled && ctx->library_root) {
        ...
        for (size_t i = 0; i < track_count && !ctx->cancelled; i++) {
            if (!tracks[i].path || !tracks[i].album_path)
                continue;
            char *audio_path
                = g_build_filename(ctx->library_root, tracks[i].album_path, tracks[i].path, NULL);
        ...
        if (!tag_album && track_count > 0 && tracks[0].album_path) {
            ...
            if (last_sep && last_sep > apath) {
                ...
                if (paren && paren > tag_album && *(paren - 1) == ' ' && strlen(paren) == 6
                    && paren[5] == ')' && g_ascii_isdigit(paren[1]) && g_ascii_isdigit(paren[2])
        ...
        if (isrc_count >= 2) {
            ...
            quadrature_result_t isrc_res
                = mb_backend_isrc_lookup(be, conn, isrcs, isrc_count, &isrc_response);
            if (isrc_res == QUADRATURE_OK && isrc_response.count > 0) {
                tally_votes(&isrc_response,
                            rg_counts,
                            rg_best_release,
                            release_counts,
                            &best_rg,
                ...
                if (confidence >= MB_MATCH_CONFIDENCE) {
                    ...
                    g_debug("ISRC lookup resolved album %" G_GINT64_FORMAT
                            ...
                            confidence * 100,
                            isrc_count);
                    dump_vote_distribution(album_id,
                                           "ISRC",
                                           rg_counts,
                                           rg_best_release,
                                           release_counts,
    ...
    if (!best_release && !ctx->cancelled && (be->caps & MB_CAP_SOLR_SEARCH) && tag_album
        && tag_artist) {
        mb_backend_solr_search(
            be, conn, tag_album, tag_artist, track_count, total_duration_ms, &best_release);
        if (best_release) {
            tier = RESOLVE_TIER_SOLR;
            g_debug("Solr search resolved album %" G_GINT64_FORMAT " '%s' by '%s' → %s",
                    album_id,
                    tag_album,
                    tag_artist,
                    best_release);
    ...
    if (!best_release && !isrc_resolved && !ctx->cancelled && fingerprint_capable
        && (be->caps & MB_CAP_FINGERPRINT)) {
        size_t tracks_to_check = track_count < (size_t)MB_FINGERPRINT_TRACKS
                                     ? track_count
                                     : (size_t)MB_FINGERPRINT_TRACKS;

        for (size_t i = 0; i < tracks_to_check && !ctx->cancelled; i++) {
            if (!tracks[i].path)
                continue;
            if (!ctx->library_root || !tracks[i].album_path)
                continue;
            char *audio_path
                = g_build_filename(ctx->library_root, tracks[i].album_path, tracks[i].path, NULL);
            ...
            g_free(audio_path);
            if (fp_res != QUADRATURE_OK)
                continue;
            fingerprinted++;

            mb_acoustid_response_t response;
            quadrature_result_t lookup_res
                = mb_backend_fingerprint_lookup(be, conn, &fp, &response);
        ...
        if (fingerprinted > 0 && best_rg_count > 0) {
            ...
            if (fingerprinted == 1) {
                ...
                if ((be->caps & MB_CAP_SOLR_SEARCH) && tag_album && tag_artist) {
                    char *solr_rel = NULL;
                    mb_backend_solr_search(
                        be, conn, tag_album, tag_artist, track_count, total_duration_ms, &solr_rel);
                    if (solr_rel) {
                        g_free(solr_rel);
                        const char *rel = g_hash_table_lookup(rg_best_release, best_rg);
                        if (rel)
...
static void
match_tracks(const mb_release_t *release,
             const db_track_t *local_tracks,
             size_t local_count,
             track_match_t **matches_out,
             size_t *match_count_out)
{
    size_t max_matches
        = local_count < release->recording_count ? local_count : release->recording_count;
    ...
    for (size_t i = 0; i < release->recording_count && matched < max_matches; i++) {
        if (release->recordings[i].position <= 0)
            continue;
        for (size_t j = 0; j < local_count; j++) {
            if (local_used[j])
                continue;
            if (local_tracks[j].track_num <= 0)
                continue;
    ...
    for (size_t i = 0; i < release->recording_count && matched < max_matches; i++) {
        ...
        for (size_t j = 0; j < local_count; j++) {
            ...
            double title_score
                = title_similarity(release->recordings[i].title, local_tracks[j].title);
...
static void
album_work_free(album_work_t *w)
{
    if (!w)
        return;
    if (w->owned_credits) {
        for (size_t m = 0; m < w->match_count; m++) {
            if (!w->owned_credits[m])
                continue;
    ...
    if (w->tracks)
        db_tracks_free(w->tracks, w->track_count);
    g_free(w->matches);
}
 ...
 */
static void
resolve_album_prepare(mb_resolver_t *ctx,
                      int64_t album_id,
                      mb_release_t *release,
                      mb_recording_link_row_t **links,
                      size_t link_count,
                      GHashTable *artist_cache,
                      ...
                      album_work_t *work)
{
    ...
    quadrature_result_t res
        = db_get_tracks_by_album(ctx->db, album_id, &work->tracks, &work->track_count);
    t1 = profile_now_ns();
    ...
    if (res != QUADRATURE_OK || work->track_count == 0) {
        ...
        if (work->tracks) {
            db_tracks_free(work->tracks, work->track_count);
            work->tracks = NULL;
        }
        work->track_count = 0;
        g_mutex_lock(&ctx->progress_mutex);
    ...
    t0 = profile_now_ns();
    match_tracks(release, work->tracks, work->track_count, &work->matches, &work->match_count);
    t1 = profile_now_ns();
        ...
        if (album_artist_id == 0) {
            ...
            if (artist_cache && album_artist_id > 0)
                g_hash_table_insert(artist_cache,
                                    ...
                                    GSIZE_TO_POINTER((gsize)album_artist_id));
        }
    }
    t1 = profile_now_ns();
    ...
    for (size_t m = 0; m < work->match_count; m++) {
        ...
        if (rec->artist_count > 0) {
            ...
            size_t ta_count = 0;
            mb_credits_to_track_artists(
                ctx->db, rec->artists, rec->artist_count, &ta, &ta_count, artist_cache);
    ...
    work->desired = (desired_album_state_t){
        ...
        .present_fields
        = DESIRED_ALBUM_TITLE | DESIRED_ALBUM_MB_RELEASE_ID | DESIRED_ALBUM_MB_RELEASE_GROUP
 ...
 */
static void
resolve_album_write_meta(mb_resolver_t *ctx, const album_work_t *w, mb_profile_stats_t *stats)
{
    if (!ctx->meta_db || !w->valid)
        return;

    int64_t t0 = profile_now_ns();
    mb_release_t *release = w->release;
    db_meta_upsert_release(ctx->meta_db,
                           release->id,
                           release->date,
                           release->type,
                           release->label,
    ...
    for (size_t m = 0; m < w->match_count; m++) {
        ...
        db_meta_upsert_recording(
            ctx->meta_db, rec->id, release->id, rec->disc_number, rec->position);
    ...
    for (size_t i = 0; i < w->link_count; i++) {
        ...
        if (!g_hash_table_contains(seen_link_types, l->link_type_gid)) {
            db_meta_upsert_link_type(
                ctx->meta_db, l->link_type_gid, l->link_type_name, l->link_type_desc);
            g_hash_table_add(seen_link_types, (gpointer)l->link_type_gid);
        }
        if (!g_hash_table_contains(seen_artists, l->artist_mbid)) {
            db_meta_upsert_artist(
                ctx->meta_db, l->artist_mbid, l->artist_name, l->artist_sort_name, l->artist_type);
...
static int
fp_claim_slot(mb_backend_t *be)
{
    gpointer stored = g_private_get(&fp_slot_key);
    if (stored)
        return GPOINTER_TO_INT(stored) - 1;
...
static void
fp_worker(gpointer data, gpointer user_data)
{
    fp_work_t *work = data;
    ...
    char *release_id = find_release_by_fingerprint(ctx,
                                                   work->album_id,
                                                   ctx->backend_fp_pool,
                                                   conn,
...
prefetch_thread_func(gpointer data)
{
    ...
    pf->releases = NULL;
    pf->links = NULL;
    pf->result = mb_backend_batch_fetch(pf->be,
                                        pf->conn,
 ...
 */
static size_t
triage_batch(mb_resolver_t *ctx,
             resolve_queue_item_t **batch,
             size_t batch_count,
             ...
             int64_t **out_album_ids)
{
    ...
    for (size_t i = 0; i < batch_count; i++) {
        if (batch[i]->release_id) {
            ...
            db_reconcile_album_mb_status(
                ctx->db, batch[i]->album_id, MB_STATUS_FAILED, (int64_t)time(NULL));
            g_mutex_lock(&ctx->progress_mutex);
            ctx->progress.albums_failed++;
            ...
            db_reconcile_album_mb_status(
                ctx->db, batch[i]->album_id, MB_STATUS_NO_MATCH, (int64_t)time(NULL));
            g_mutex_lock(&ctx->progress_mutex);
...
static void
triage_free(char **release_ids, int64_t *album_ids, size_t count)
{
    for (size_t i = 0; i < count; i++)
 ...
 */
static void
write_resolve_batch(mb_resolver_t *ctx,
                    const char *const *release_ids,
                    int64_t *album_ids,
                    size_t release_count,
                    GHashTable *releases,
                    GHashTable *all_links,
                    mb_profile_stats_t *stats)
{
    GHashTable *artist_cache = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);

    if (ctx->meta_db)
        db_meta_begin(ctx->meta_db);
    ...
    for (size_t i = 0; i < release_count && !ctx->cancelled; i++) {
        mb_release_t *release = releases ? g_hash_table_lookup(releases, release_ids[i]) : NULL;

        if (!release) {
            db_reconcile_album_mb_status(
                ctx->db, album_ids[i], MB_STATUS_FAILED, (int64_t)time(NULL));
            g_mutex_lock(&ctx->progress_mutex);
            ctx->progress.albums_failed++;
            ctx->progress.albums_processed++;
            g_mutex_unlock(&ctx->progress_mutex);
        ...
        resolve_album_prepare(
            ctx, album_ids[i], release, links, link_count, artist_cache, stats, &works[i]);
    }
    ...
    if (batch_n > 0) {
        db_reconcile_albums(
            ctx->db, batch_ids, batch_desireds, batch_n, &RECONCILE_POLICY_MB, NULL);
    }
    g_free(batch_ids);
...
static void
process_resolve_batch(mb_resolver_t *ctx,
                      resolve_queue_item_t **batch,
                      size_t batch_count,
                      mb_profile_stats_t *stats)
{
    ...
    size_t release_count = triage_batch(ctx, batch, batch_count, &release_ids, &album_ids);
    t1 = profile_now_ns();
    ...
    t0 = profile_now_ns();
    mb_backend_batch_fetch(ctx->backend,
                           ctx->backend_conn,
                           (const char **)(void *)release_ids,
                           release_count,
                           &releases,
                           &all_links);
    t1 = profile_now_ns();
    ...
    if (all_links)
        g_hash_table_destroy(all_links);
...
quadrature_result_t
mb_resolver_create(mb_resolver_t **out,
                   quadrature_db_t *db,
                   ...
                   void *user_data)
{
    ...
    ctx->data_root = options->data_root
                         ? g_strdup(options->data_root)
                         : (options->library_root ? g_strdup(options->library_root) : NULL);
    ctx->acoustid_index_url
        = options->acoustid_index_url ? g_strdup(options->acoustid_index_url) : NULL;
    ctx->solr_url = options->mb_solr_url ? g_strdup(options->mb_solr_url) : NULL;
    g_mutex_init(&ctx->progress_mutex);

    /* Build backend URI + config based on PG-vs-HTTP selection. */
    char *uri = use_pg ? g_strdup_printf("pg://%s", options->pg_conninfo) : g_strdup("mb+https://");
    ...
    if (ctx->data_root) {
        if (db_meta_open_attached(ctx->db, ctx->data_root, &ctx->meta_db) != QUADRATURE_OK) {
            g_warning(
                "mb_resolver_create: failed to attach metadata DB — relations will not be written");
            ctx->meta_db = NULL;
        }
    }
...
quadrature_result_t
mb_resolver_run(mb_resolver_t *ctx)
{
    if (!ctx)
        return QUADRATURE_ERROR_INVALID_PARAM;
    ...
    quadrature_result_t res
        = db_get_unresolved_albums(ctx->db, retry_before, &album_ids, &album_count);
    if (res != QUADRATURE_OK)
        return res;
    ...
    for (size_t i = 0; i < album_count && !ctx->cancelled; i++) {
        ...
        const char *rid
            = (album && album->musicbrainz_release_id && album->musicbrainz_release_id[0])
    ...
    bool has_fingerprint_support = resolver_use_pg ? (ctx->options.acoustid_pg_conninfo
    ...
    if (untagged_ids->len > 0 && has_fingerprint_support && !ctx->cancelled) {
        ...
        int parallelism
            = ctx->options.parallelism > 0 ? ctx->options.parallelism : (int)g_get_num_processors();
        if (parallelism > 8)
            parallelism = 8;
        if (parallelism > (int)untagged_ids->len)
            parallelism = (int)untagged_ids->len;
        ...
        char *fp_uri = fp_use_pg ? g_strdup_printf("pg://%s", ctx->options.pg_conninfo)
                                 : g_strdup("mb+https://");
        ...
        if (res == QUADRATURE_OK) {
            ...
            g_warning(
                "MB resolver: failed to create fingerprint backend pool, fingerprinting disabled");
    ...
    while (total_processed < album_count && !ctx->cancelled) {
        ...
        t0 = profile_now_ns();
        resolve_queue_item_t *item
            = g_async_queue_timeout_pop(resolve_queue, QUEUE_DRAIN_TIMEOUT_US);
        if (!item) {
            ...
            if (!fp_pool || g_thread_pool_unprocessed(fp_pool) == 0) {
                ...
                if (!item)
                    break; // Queue truly empty and no more producers
            } else {
        ...
        size_t release_count
            = triage_batch(ctx, batch, batch_count, &release_ids, &batch_album_ids);
        t1 = profile_now_ns();
        ...
        if (release_count == 0) {
            ...
            if (consecutive_pg_failures >= 3) {
                g_warning("Phase 6: circuit breaker tripped — skipping remaining albums after 3 "
                          "consecutive PG failures");
                /* Write whatever we have (NULL releases → albums marked FAILED) */
                write_resolve_batch(ctx,
                                    (const char *const *)pending_ids,
                                    pending_album_ids,
                                    pending_count,
                                    pf.releases,
                                    pf.links,
                ...
                prof.batch_count++;
                if (pf.releases)
                    g_hash_table_destroy(pf.releases);
                if (pf.links)
                    g_hash_table_destroy(pf.links);
            ...
            if (prev_releases)
                g_hash_table_destroy(prev_releases);
            if (prev_links)
                g_hash_table_destroy(prev_links);
            ...
            if (fetch_res != QUADRATURE_OK) {
                ...
                lnk = NULL;
                t0 = profile_now_ns();
                fetch_res = mb_backend_batch_fetch(ctx->backend,
                                                   ctx->backend_conn,
                                                   (const char **)(void *)release_ids,
                                                   release_count,
                                                   &rel,
    ...
    if (prof.albums_written > 0) {
        ...
        g_info("=== MB Resolver Profile (%zu albums in %zu batches) ===",
               prof.albums_written,
               prof.batch_count);
        g_info("  Batch-level:  queue_wait=%.0fms  triage=%.0fms  pg_fetch=%.0fms  "
               "sqlite_write=%.0fms",
               (double)prof.queue_wait_ns / 1e6,
               (double)prof.triage_ns / 1e6,
               (double)prof.pg_fetch_ns / 1e6,
               write_ms);
        g_info("  Per-album avg (%.2fms total):", write_ms / n);
...

    {
        ...
        size_t n_acoust
            = __atomic_load_n(&ctx->tier_count[RESOLVE_TIER_ACOUSTID], __ATOMIC_RELAXED);
        ...
        double isrc_ms
            = (double)__atomic_load_n(&ctx->tier_ns[RESOLVE_TIER_ISRC], __ATOMIC_RELAXED) / 1e6;
        double solr_ms
            = (double)__atomic_load_n(&ctx->tier_ns[RESOLVE_TIER_SOLR], __ATOMIC_RELAXED) / 1e6;
        double acoust_ms
            = (double)__atomic_load_n(&ctx->tier_ns[RESOLVE_TIER_ACOUSTID], __ATOMIC_RELAXED) / 1e6;
        ...
        g_message("=== Resolution Summary — %s (%zu albums) ===",
                  ctx->library_root ? ctx->library_root : "(unknown)",
                  n_total);
        ...
        g_message("  AcoustID:       %zu resolved    (%.1f albums/sec)",
                  n_acoust,
                  acoust_ms > 0.0 ? (double)(untagged - n_isrc - n_solr) / (acoust_ms / 1000.0)
                                  : 0.0);
...
void
mb_resolver_destroy(mb_resolver_t *ctx)
{
    if (!ctx)
        return;
    if (ctx->backend)
        mb_backend_destroy(ctx->backend);
    if (ctx->backend_prefetch)
        mb_backend_destroy(ctx->backend_prefetch);
    if (ctx->backend_fp_pool)
        mb_backend_destroy(ctx->backend_fp_pool);
    if (ctx->meta_db) {
