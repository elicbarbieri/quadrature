...
quadrature_result_t mb_pg_install_batch_function(mb_pg_client_t* client) {
    if (!client || !client->conn) return QUADRATURE_ERROR_INVALID_PARAM;
...
static mb_artist_t* flatten_artists(GPtrArray* ptrs) {
    if (!ptrs || ptrs->len == 0) {
        if (ptrs) g_ptr_array_free(ptrs, TRUE);
        return NULL;
    }
 ...
 */
static void flush_recording(mb_recording_t* rec, GPtrArray* artists,
                              GPtrArray* recs_arr) {
    if (!rec) return;
    if (artists) {
    ...
    if (recs_arr) g_ptr_array_add(recs_arr, rec);
}

/**
 * Finalize recordings: move from per-release GPtrArray into release struct.
 */
static void finalize_recordings(GHashTable* releases, GHashTable* rec_arrays) {
    GHashTableIter iter;
    gpointer key, val;
...
quadrature_result_t mb_fetch_all_batch(mb_pg_client_t* client,
    ...
    *out_links = NULL;
    ...
    if (PQresultStatus(res) != PGRES_TUPLES_OK) {
        g_warning("mb_fetch_all_batch: PG query failed: %s",
            ...
            if (new_recording) {
                ...
                if (!t_release_gid || strcmp(rel_gid, t_release_gid) != 0) {
                    ...
                    if (!cur_recs_arr) {
                        ...
                        mb_release_t* r = g_hash_table_lookup(releases, rel_gid);
                        if (r) g_hash_table_insert(rec_arrays, r->id, cur_recs_arr);
                    }
