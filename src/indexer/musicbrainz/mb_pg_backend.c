...
static quadrature_result_t pg_pool_create(const mb_backend_config_t* cfg,
                                           size_t slot_count,
                                           mb_pool_t** out)
{
    g_assert(cfg != NULL);
    ...
    state->next_slot = 0;
    state->mb_solr_url = (cfg->mb_solr_url && cfg->mb_solr_url[0])
                         ? g_strdup(cfg->mb_solr_url) : NULL;
...
static void pg_pool_destroy(mb_pool_t* pool) {
    if (!pool) return;
    pg_pool_state_t* state = (pg_pool_state_t*)pool;

    for (size_t i = 0; i < state->count; i++) {
        mb_pg_client_destroy(state->slots[i].mb_conn);
        if (state->slots[i].acoustid_conn) mb_pg_client_destroy(state->slots[i].acoustid_conn);
        if (state->slots[i].fp_index_conn) mb_http_conn_destroy(state->slots[i].fp_index_conn);
    }
    g_free(state->slots);
...
static mb_conn_t* pg_pool_claim_slot(mb_pool_t* pool, int slot) {
    g_assert(pool != NULL);
    pg_pool_state_t* state = (pg_pool_state_t*)pool;
...
static bool pg_conn_reset(mb_conn_t* conn) {
    g_assert(conn != NULL);
    pg_slot_t* s = (pg_slot_t*)conn;
    bool ok = mb_pg_client_reset(s->mb_conn);
    if (s->acoustid_conn && !mb_pg_client_reset(s->acoustid_conn)) ok = false;
    return ok;
}

static quadrature_result_t pg_batch_fetch(mb_conn_t* conn,
...
static quadrature_result_t pg_isrc_lookup(mb_conn_t* conn,
...
static quadrature_result_t pg_fingerprint_lookup(mb_conn_t* conn,
...
static quadrature_result_t pg_solr_search(mb_conn_t* conn,
                                           ...
                                           char** out_release_id)
{
    ...
    *out_release_id = mb_solr_search_release(s->mb_conn, s->pool->mb_solr_url,
...
static size_t pg_pool_count(const mb_pool_t* pool) {
    g_assert(pool != NULL);
    return ((const pg_pool_state_t*)pool)->count;
}

static int pg_pool_claim_round_robin(mb_pool_t* pool) {
    g_assert(pool != NULL);
    pg_pool_state_t* s = (pg_pool_state_t*)pool;
    return g_atomic_int_add(&s->next_slot, 1) % (int)s->count;
}

static const char* pg_name(const mb_pool_t* pool) {
    (void)pool;
    return "pg";
}
                                          ...
                                          mb_backend_t** out)
{
    ...
    if (res != QUADRATURE_OK) {
        if (pool) pg_pool_destroy(pool);
        return res;
    }

    mb_caps_t caps = MB_CAP_BATCH_FETCH | MB_CAP_ISRC_LOOKUP | MB_CAP_FINGERPRINT
                   | MB_CAP_PREFETCH;
