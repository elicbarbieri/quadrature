...
// Image extensions
static const char *IMAGE_EXTS[] = { ".jpg", ".jpeg", ".png", ".webp", NULL };

// Audio extensions for embedded art
static const char *AUDIO_EXTS[] = { ".mp3", ".flac", ".m4a", NULL };
...
static int
has_extension_in_list(const char *filename, const char **ext_list)
{
...
static int
get_art_priority(const char *filename)
{
 ...
 */
quadrature_result_t
artwork_find(const char *album_dir, char *art_path, size_t path_size)
{
 ...
 */
quadrature_result_t
artwork_find_bytes(const char *album_dir, uint8_t **data_out, size_t *size_out)
{
    ...
    while ((dent = readdir(dir)) != NULL && attempts < 2) {
        if (dent->d_name[0] == '.')
            continue;
        if (!has_audio_extension(dent->d_name))
            continue;
        attempts++;
...
vips_resize_buffer(const void *input_data,
                   size_t input_size,
                   int thumb_size,
                   uint8_t **output_data,
                   size_t *output_size)
{
    ...
    VipsImage *img = NULL;
    if (vips_thumbnail_buffer((void *)input_data,
                              input_size,
                              &img,
                              thumb_size,
                              "height",
...
artwork_atlas_reader_t *
artwork_atlas_reader_open(const char *path)
{
    ...
    if (memcmp(h->magic, ARTWORK_ATLAS_MAGIC, ARTWORK_ATLAS_MAGIC_SIZE) != 0 || h->version < 2
    ...
    if (h->count > 0) {
        ...
        reader->pixel_data
            = (const uint8_t *)reader->album_ids + (size_t)h->count * sizeof(int64_t);
    }

    /* v3: no_art section follows pixel data (pointers into mmap) */
    if (h->version >= 3 && h->count > 0) {
        const uint8_t *after_pixels = reader->pixel_data + (size_t)h->count * reader->pixel_stride;
        uint32_t nac;
...
void
artwork_atlas_reader_close(artwork_atlas_reader_t *reader)
{
    if (!reader)
        return;
...
int32_t
artwork_atlas_reader_lookup(const artwork_atlas_reader_t *reader, int64_t album_id)
{
    if (!reader || !reader->album_ids)
        return -1;
...
uint32_t
artwork_atlas_reader_get_thumb_size(const artwork_atlas_reader_t *reader)
{
...
const uint8_t *
artwork_atlas_reader_get_pixels_at(const artwork_atlas_reader_t *reader, size_t index)
{
    if (!reader || !reader->pixel_data || index >= reader->header.count)
        return NULL;
    return reader->pixel_data + index * reader->pixel_stride;
}

size_t
artwork_atlas_reader_get_file_size(const artwork_atlas_reader_t *reader)
{
...
static int
compare_atlas_entries(const void *a, const void *b)
{
    const atlas_build_entry_t *ea = a;
    const atlas_build_entry_t *eb = b;
    if (ea->album_id < eb->album_id)
        return -1;
    if (ea->album_id > eb->album_id)
        return 1;
    return 0;
 ...
 */
static uint8_t *
generate_fallback_pixels(uint32_t pixel_stride)
{
...
quadrature_result_t
artwork_atlas_builder_create(const char *atlas_path, int thumb_size, artwork_atlas_builder_t **out)
{
    if (!atlas_path || !out) {
    ...
    builder->pixel_stride
        = (uint32_t)builder->thumb_size * builder->thumb_size * ARTWORK_ATLAS_CHANNELS;
...
void
artwork_atlas_builder_set_fanart_covers_dir(artwork_atlas_builder_t *builder, const char *dir)
{
    if (!builder)
        return;
...
void
artwork_atlas_builder_set_album_rg_map(artwork_atlas_builder_t *builder,
                                       GHashTable *album_id_to_rg_id)
{
    if (!builder)
        return;
    if (builder->album_rg_map)
        g_hash_table_destroy(builder->album_rg_map);
    builder->album_rg_map = album_id_to_rg_id; // takes ownership
}

quadrature_result_t
artwork_atlas_builder_load_existing(artwork_atlas_builder_t *builder)
{
    if (!builder)
        return QUADRATURE_ERROR_INVALID_PARAM;
    ...
    if (!f)
        return QUADRATURE_OK; // No existing atlas — start fresh
    ...
    if (memcmp(header.magic, ARTWORK_ATLAS_MAGIC, ARTWORK_ATLAS_MAGIC_SIZE) != 0
        ...
        fclose(f);
        return QUADRATURE_OK; // Incompatible — start fresh
    }
    ...
    // Validate CRC32
    {
        ...
        if (fread(buf, 1, body_size, f) != body_size) {
            g_free(buf);
            fclose(f);
            return QUADRATURE_OK;
        }
    ...
    for (uint32_t i = 0; i < header.count; i++) {
        ...
        if (is_v2 && pixel_stride == builder->pixel_stride) {
            is_placeholder
                = (memcmp(pixels + (size_t)i * pixel_stride, builder->fallback_pixels, pixel_stride)
                   == 0);
        }
        ...
        if (is_placeholder) {
            ...
            if (builder->no_art_count >= builder->no_art_capacity) {
                ...
                builder->no_art_ids
                    = g_realloc(builder->no_art_ids, builder->no_art_capacity * sizeof(int64_t));
            }
    ...
    if (!is_v2) {
        ...
        if (fread(&no_art_count, sizeof(no_art_count), 1, f) == 1 && no_art_count > 0) {
            ...
            size_t read = fread(
                builder->no_art_ids + builder->no_art_count, sizeof(int64_t), no_art_count, f);
...
quadrature_result_t
artwork_atlas_builder_add_no_art(artwork_atlas_builder_t *builder, int64_t album_id)
{
    if (!builder)
        return QUADRATURE_ERROR_INVALID_PARAM;
...
size_t
artwork_atlas_builder_sweep_no_art(artwork_atlas_builder_t *builder, GArray *promoted_ids)
{
    ...
    for (size_t src = 0; src < builder->no_art_count; src++) {
        const char *rg_id = g_hash_table_lookup(builder->album_rg_map, &builder->no_art_ids[src]);
        if (rg_id) {
            char path[INDEXER_PATH_MAX];
            g_snprintf(path, sizeof(path), "%s/%s.jpg", builder->fanart_covers_dir, rg_id);
 ...
 */
static quadrature_result_t
find_and_resize_artwork(const char *album_dir,
                        int thumb_size,
                        uint8_t **pixels_out,
                        size_t *size_out,
                        int64_t *find_ns,
                        int64_t *resize_ns)
{
    ...
    g_free(art_data);
    if (resize_ns)
        *resize_ns = profile_now_ns() - t1;
    return res;
}

quadrature_result_t
artwork_atlas_process_album(artwork_atlas_builder_t *builder,
                            int64_t album_id,
                            ...
                            bool *used_fallback)
{
    ...
    size_t pixel_size = 0;
    quadrature_result_t res = find_and_resize_artwork(
        album_dir, builder->thumb_size, &pixel_data, &pixel_size, &find_ns, &resize_ns);
    ...
    if (res != QUADRATURE_OK) {
        ...
        if (builder->fanart_covers_dir && builder->album_rg_map) {
            ...
            if (rg_id) {
                g_snprintf(fanart_path,
                           sizeof(fanart_path),
                           "%s/%s.jpg",
                           builder->fanart_covers_dir,
                           rg_id);
            } else {
                fanart_path[0] = '\0';
            }
            ...
            if (g_file_test(fanart_path, G_FILE_TEST_EXISTS)) {
                ...
                if (g_file_get_contents(fanart_path, &contents, &length, NULL) && length > 0) {
                    int64_t t1 = profile_now_ns();
                    res = vips_resize_buffer((const void *)contents,
                                             length,
                                             builder->thumb_size,
                                             &pixel_data,
                                             &pixel_size);
                                   ...
                                   size_t *errors_out)
{
    if (!builder)
        return;
    if (processed_out)
        *processed_out = atomic_load(&builder->processed_count);
    if (errors_out)
        *errors_out = atomic_load(&builder->error_count);
}

void
artwork_atlas_builder_get_profile(artwork_atlas_builder_t *builder, artwork_atlas_profile_t *out)
{
    if (!builder || !out)
        return;
...
artwork_atlas_builder_finish(artwork_atlas_builder_t *builder, bool *out_changed)
{
    ...
    qsort(
        builder->entries, builder->entry_count, sizeof(atlas_build_entry_t), compare_atlas_entries);
    qsort(builder->no_art_ids, builder->no_art_count, sizeof(int64_t), compare_int64);
...
void
artwork_atlas_builder_destroy(artwork_atlas_builder_t *builder)
{
    if (!builder)
        return;
