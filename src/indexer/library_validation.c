 ...
 */
static bool
uses_continuous_numbering(const metadata_result_t *mr)
{
    if (mr->track_count == 0)
        return false;
 ...
 */
static void
check_global_track_sequence(indexer_t *idx, const metadata_result_t *mr)
{
    if (!idx || !mr || !mr->tracks || !mr->dir_path)
        return;
    if (mr->track_count == 0)
        return;
    ...
    for (uint16_t t = 1; t <= max_track; t++) {
        ...
        while (t + 1 <= max_track && !present[t + 1])
            t++;
 ...
 */
static void
check_per_disc_track_gaps(indexer_t *idx, const metadata_result_t *mr)
{
    if (!idx || !mr || !mr->tracks || !mr->dir_path)
        return;
    if (mr->track_count == 0)
    ...
    for (uint16_t disc = 0; disc <= max_disc; disc++) {
        ...
        if (disc_track_count == 0 || max_track == 0)
            continue;

        /* No gaps possible if count equals max */
        if (disc_track_count == max_track)
            continue;
 ...
 */
static void
check_duplicate_track_positions(indexer_t *idx, const metadata_result_t *mr)
{
    if (!idx || !mr || !mr->tracks || !mr->dir_path)
        return;
    if (mr->track_count < 2)
        ...
        for (size_t j = i + 1; j < mr->track_count; j++) {
            if (mr->tracks[i].disc_num == mr->tracks[j].disc_num
                ...
                log_indexer_error(idx,
                                  ...
                                  mr->tracks[i].disc_num,
                                  mr->tracks[i].track_num,
                                  path_i,
                                  path_j);
...
void
validate_album_track_numbering(indexer_t *idx, const metadata_result_t *mr)
{
    if (!idx || !mr)
        return;
    if (mr->track_count == 0)
