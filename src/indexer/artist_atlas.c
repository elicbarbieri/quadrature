...
bool
mbid_parse(const char *str, uint8_t out[ARTIST_ATLAS_UUID_SIZE])
{
    if (!str)
        return false;
...
static int
compare_uuids(const void *a, const void *b)
{
    return memcmp(a, b, ARTIST_ATLAS_UUID_SIZE);
}

static int
compare_art_entries(const void *a, const void *b)
{
...
quadrature_result_t
artist_atlas_builder_create(const char *atlas_path, int thumb_size, artist_atlas_builder_t **out)
{
    if (!atlas_path || thumb_size <= 0 || !out)
...
quadrature_result_t
artist_atlas_builder_load_existing(artist_atlas_builder_t *builder)
{
    if (!builder)
        return QUADRATURE_ERROR_INVALID_PARAM;
    ...
    if (fd < 0)
        return QUADRATURE_OK; // No existing atlas — that's fine

    struct stat st;
    if (fstat(fd, &st) < 0
        || (size_t)st.st_size < sizeof(artist_atlas_header_t) + ATLAS_CRC32_SIZE) {
        close(fd);
        return QUADRATURE_OK;
    }
    ...
    // Validate file size
    size_t expected_min = sizeof(*h) + (size_t)h->art_count * ARTIST_ATLAS_UUID_SIZE
                          + (size_t)h->art_count * pixel_stride + sizeof(uint32_t)
                          + (size_t)h->no_art_count * ARTIST_ATLAS_UUID_SIZE + ATLAS_CRC32_SIZE;
    ...
    munmap(map, st.st_size);
    g_info("artist_atlas: loaded existing atlas (%u art, %u no-art)", loaded_art, loaded_no_art);
    return QUADRATURE_OK;
}
 ...
 */
static int
find_art_entry(artist_atlas_builder_t *b, const uint8_t uuid[16])
{
...
static int
find_no_art_entry(artist_atlas_builder_t *b, const uint8_t uuid[16])
{
...
quadrature_result_t
artist_atlas_builder_add_art(artist_atlas_builder_t *builder,
                             ...
                             size_t pixel_size)
{
    ...
    if (builder->art_count >= builder->art_capacity) {
        builder->art_capacity *= 2;
        builder->art_entries
            = g_realloc(builder->art_entries, builder->art_capacity * sizeof(artist_atlas_entry_t));
    }
...
quadrature_result_t
artist_atlas_builder_add_no_art(artist_atlas_builder_t *builder, const uint8_t uuid[16])
{
    if (!builder || !uuid)
        return QUADRATURE_ERROR_INVALID_PARAM;

    // Don't add if already in art list (has real artwork)
    if (find_art_entry(builder, uuid) >= 0)
        return QUADRATURE_OK;

    // Don't add duplicates
    if (find_no_art_entry(builder, uuid) >= 0)
        return QUADRATURE_OK;

    if (builder->no_art_count >= builder->no_art_capacity) {
        builder->no_art_capacity *= 2;
        builder->no_art_uuids
            = g_realloc(builder->no_art_uuids, builder->no_art_capacity * ARTIST_ATLAS_UUID_SIZE);
    }
...
artist_atlas_builder_finish(artist_atlas_builder_t *builder)
{
    ...
    qsort(builder->art_entries,
          builder->art_count,
          sizeof(artist_atlas_entry_t),
          compare_art_entries);
    qsort(builder->no_art_uuids, builder->no_art_count, ARTIST_ATLAS_UUID_SIZE, compare_uuids);
    ...
    if (builder->no_art_count > 0) {
        if (fwrite(builder->no_art_uuids, ARTIST_ATLAS_UUID_SIZE, builder->no_art_count, f)
            != builder->no_art_count)
            goto write_error;
    }
    ...
    if (rename(builder->temp_path, builder->atlas_path) != 0) {
        g_warning("artist_atlas: rename failed: %s -> %s", builder->temp_path, builder->atlas_path);
...
void
artist_atlas_builder_destroy(artist_atlas_builder_t *builder)
{
    if (!builder)
        return;
    for (size_t i = 0; i < builder->art_count; i++)
...
artist_atlas_reader_t *
artist_atlas_reader_open(const char *path)
{
    if (!path)
        return NULL;
    ...
    size_t expected = sizeof(*h) + (size_t)h->art_count * ARTIST_ATLAS_UUID_SIZE
                      + (size_t)h->art_count * pixel_stride + sizeof(uint32_t)
                      + (size_t)h->no_art_count * ARTIST_ATLAS_UUID_SIZE + ATLAS_CRC32_SIZE;
    ...
    g_info("artist_atlas: opened %s (%u art, %u no-art)", path, h->art_count, h->no_art_count);
    return r;
}

void
artist_atlas_reader_close(artist_atlas_reader_t *reader)
{
    if (!reader)
        return;
 ...
 */
static int32_t
uuid_binary_search(const uint8_t *keys, uint32_t count, const uint8_t uuid[16])
{
...
int32_t
artist_atlas_reader_lookup(const artist_atlas_reader_t *reader, const uint8_t uuid[16])
{
...
{
    if (!reader || !reader->pixel_data || index < 0 || (uint32_t)index >= reader->header->art_count)
        return NULL;
    return reader->pixel_data + (uint32_t)index * reader->pixel_stride;
}

uint32_t
artist_atlas_reader_get_thumb_size(const artist_atlas_reader_t *reader)
{
    return reader && reader->header ? reader->header->thumb_size : 0;
}

uint32_t
artist_atlas_reader_get_pixel_stride(const artist_atlas_reader_t *reader)
{
