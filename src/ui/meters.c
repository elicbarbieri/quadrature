...
static void
ui_spectrum_snapshot(GtkWidget *widget, GtkSnapshot *snap)
{
    UiSpectrum *s = UI_SPECTRUM(widget);
    int w = gtk_widget_get_width(widget);
    int h = gtk_widget_get_height(widget);
    if (w <= 0 || h <= 0 || s->num_bars <= 0)
        return;
...
static void
ui_spectrum_measure(
    GtkWidget *w, GtkOrientation o, int for_size, int *min, int *nat, int *min_bl, int *nat_bl)
{
...
static void
ui_spectrum_class_init(UiSpectrumClass *klass)
{
...
void
ui_spectrum_set_bars(UiSpectrum *s, const float *left, const float *right, int count)
{
    ...
    for (int i = 0; i < n; i++) {
        ...
        s->smoothed_left[i] = (lt > lc) ? lc + (lt - lc) * 0.5f : lc + (lt - lc) * 0.15f;
        s->bars_left[i] = lt;

        /* Right channel: fast attack, slow release */
        float rt = CLAMP(right[i], 0.0f, 1.0f);
        float rc = s->smoothed_right[i];
        s->smoothed_right[i] = (rt > rc) ? rc + (rt - rc) * 0.5f : rc + (rt - rc) * 0.15f;
        s->bars_right[i] = rt;
    }
...
#define PLAYHEAD_EDGE_FADE \
    1.0f                           /* px of soft alpha falloff on each side
...
static const GdkRGBA WAVEFORM_PLAYED_DIM
    = { 0.00f, 0.55f, 0.68f, 0.65f }; /* muted cyan when disabled */
...
static void
on_drag_begin(GtkGestureDrag *g, double x, double y, gpointer data)
{
    (void)g;
    (void)y;
...
static void
on_drag_update(GtkGestureDrag *g, double offset_x, double offset_y, gpointer data)
{
    (void)offset_y;
    UiWaveformSeekBar *w = UI_WAVEFORM_SEEK_BAR(data);
    if (!w->dragging)
    ...
    gtk_adjustment_set_value(w->adjustment, CLAMP((start_x + offset_x) / (double)width, 0.0, 1.0));
    gtk_widget_queue_draw(GTK_WIDGET(w));
}

static void
on_drag_end(GtkGestureDrag *g, double offset_x, double offset_y, gpointer data)
{
    (void)g;
    (void)offset_x;
    (void)offset_y;
    ...
    if (w->dragging) {
        w->dragging = FALSE;
        g_signal_emit(w, waveform_seek_signal_id, 0, gtk_adjustment_get_value(w->adjustment));
    }
}

/* Right-click during drag: cancel without seeking, snap back to playback position */
static void
on_cancel_click(GtkGestureClick *g, int n_press, double x, double y, gpointer data)
{
    (void)g;
    (void)n_press;
    (void)x;
...
static gboolean
on_leave_tick(GtkWidget *widget, GdkFrameClock *clock, gpointer data)
{
    (void)clock;
    UiWaveformSeekBar *w = UI_WAVEFORM_SEEK_BAR(data);
...
on_leave(GtkEventControllerMotion *ctrl, gpointer data)
{
    ...
    if (w->leave_tick_id == 0
        ...
        w->leave_tick_id = gtk_widget_add_tick_callback(GTK_WIDGET(w), on_leave_tick, w, NULL);
    }
}
...
rgba_lerp_linear(GdkRGBA a, GdkRGBA b, float t)
{
...
static void
ui_waveform_seek_bar_snapshot(GtkWidget *widget, GtkSnapshot *snap)
{
    UiWaveformSeekBar *w = UI_WAVEFORM_SEEK_BAR(widget);
    int width = gtk_widget_get_width(widget);
    int height = gtk_widget_get_height(widget);
    if (width <= 0 || height <= 0 || !w->has_data || w->num_bins <= 0)
        return;
    ...
    gboolean spotlight_active
        = sensitive && w->waveform_hovered && !w->playhead_hovered && !w->dragging;
    float spot_target = spotlight_active ? 1.0f : 0.0f;
    w->spotlight_t += (spot_target - w->spotlight_t) * SPOTLIGHT_LERP_SPEED;
    if (fabsf(w->spotlight_t - spot_target) < 0.01f)
        w->spotlight_t = spot_target;
    ...
    if (split_x > 0.0f && split_x < (float)width) {
        ...
        graphene_rect_t line
            = GRAPHENE_RECT_INIT(split_x - ph_full_w * 0.5f, cy - ph_h, ph_full_w, ph_h * 2.0f);
...
static void
ui_waveform_seek_bar_measure(
    GtkWidget *widget, GtkOrientation o, int for_size, int *min, int *nat, int *min_bl, int *nat_bl)
{
    (void)widget;
    (void)for_size;
    if (o == GTK_ORIENTATION_HORIZONTAL) {
...
static void
ui_waveform_seek_bar_dispose(GObject *obj)
{
...
static void
ui_waveform_seek_bar_class_init(UiWaveformSeekBarClass *klass)
{
...
GtkWidget *
ui_waveform_seek_bar_new(void)
{
    return g_object_new(UI_TYPE_WAVEFORM_SEEK_BAR, NULL);
}

GtkAdjustment *
ui_waveform_seek_bar_get_adjustment(UiWaveformSeekBar *w)
{
...
gboolean
ui_waveform_seek_bar_is_dragging(UiWaveformSeekBar *w)
{
...
void
ui_waveform_seek_bar_set_playback_position(UiWaveformSeekBar *w, double value)
{
...
void
ui_waveform_seek_bar_clear(UiWaveformSeekBar *w)
{
    g_return_if_fail(UI_IS_WAVEFORM_SEEK_BAR(w));
    w->has_data = FALSE;
    w->num_bins = 0;
...
gboolean
ui_waveform_seek_bar_is_animating(UiWaveformSeekBar *w)
{
    g_return_val_if_fail(UI_IS_WAVEFORM_SEEK_BAR(w), FALSE);
    return w->animating;
}
