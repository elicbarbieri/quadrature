...
enum { PROP_0, PROP_RUNNING, N_PROPS };
...
static gboolean
on_indexer_event_idle(gpointer user_data)
{
    ...
    case INDEXER_LIBRARY_UPDATED:
        g_signal_emit(
            self, signals[SIGNAL_LIBRARY_UPDATED], 0, lib, &data->progress, data->changeset);
        break;
...
static void
on_indexer_callback(indexer_event_t event,
...
static indexer_t *
create_indexer_for_scan(IndexerController *self, ScanCallbackData *cb_data)
{
...
static void
try_start_pending(IndexerController *self)
{
...
static void
indexer_controller_get_property(GObject *object, guint prop_id, GValue *value, GParamSpec *pspec)
{
...
static void
indexer_controller_dispose(GObject *object)
{
    IndexerController *self = INDEXER_CONTROLLER(object);
...
static void
indexer_controller_class_init(IndexerControllerClass *klass)
{
    ...
    props[PROP_RUNNING] = g_param_spec_boolean("running",
                                               "Running",
                                               "Whether any library scan is in progress",
                                               FALSE,
     ...
     */
    signals[SIGNAL_LIBRARY_UPDATED] = g_signal_new("library-updated",
                                                   G_TYPE_FROM_CLASS(klass),
                                                   G_SIGNAL_RUN_LAST,
                                                   0,
                                                   NULL,
     ...
     */
    signals[SIGNAL_ARTWORK_UPDATED] = g_signal_new("artwork-updated",
                                                   G_TYPE_FROM_CLASS(klass),
                                                   G_SIGNAL_RUN_LAST,
                                                   0,
                                                   NULL,
     ...
     */
    signals[SIGNAL_COMPLETED] = g_signal_new("completed",
                                             G_TYPE_FROM_CLASS(klass),
                                             G_SIGNAL_RUN_LAST,
                                             0,
                                             NULL,
     ...
     */
    signals[SIGNAL_ALL_COMPLETED] = g_signal_new("all-completed",
                                                 G_TYPE_FROM_CLASS(klass),
                                                 G_SIGNAL_RUN_LAST,
                                                 0,
                                                 NULL,
...
static void
indexer_controller_init(IndexerController *self)
{
    self->thread_count = 0;
...
IndexerController *
indexer_controller_new(void)
{
    return g_object_new(INDEXER_TYPE_CONTROLLER, NULL);
}

void
indexer_controller_set_thread_count(IndexerController *self, int thread_count)
{
...
void
indexer_controller_set_process_artwork(IndexerController *self, gboolean enable)
{
    g_return_if_fail(INDEXER_IS_CONTROLLER(self));
...
void
indexer_controller_set_art_size(IndexerController *self, int size)
{
    g_return_if_fail(INDEXER_IS_CONTROLLER(self));
...
void
indexer_controller_set_max_concurrent(IndexerController *self, int max_concurrent)
{
    g_return_if_fail(INDEXER_IS_CONTROLLER(self));
...
gboolean
indexer_controller_start(IndexerController *self,
    ...
    if (data_roots) {
        for (gsize i = 0; i < path_count; i++) {
            if (data_roots[i] && g_strcmp0(data_roots[i], library_roots[i]) != 0)
                g_hash_table_insert(
                    self->data_root_map, g_strdup(library_roots[i]), g_strdup(data_roots[i]));
        }
    }
...
void
indexer_controller_cancel(IndexerController *self)
{
    g_return_if_fail(INDEXER_IS_CONTROLLER(self));
...
void
indexer_controller_cancel_library(IndexerController *self, const char *library_path)
{
    g_return_if_fail(INDEXER_IS_CONTROLLER(self));
...
gboolean
indexer_controller_is_running(IndexerController *self)
{
    g_return_val_if_fail(INDEXER_IS_CONTROLLER(self), FALSE);
    return self->running;
}
...
void
indexer_controller_set_musicbrainz_resolve(IndexerController *self, gboolean enable)
{
    g_return_if_fail(INDEXER_IS_CONTROLLER(self));
...
void
indexer_controller_set_pg_conninfo(IndexerController *self, const char *conninfo)
{
    g_return_if_fail(INDEXER_IS_CONTROLLER(self));
    g_free(self->pg_conninfo);
...
void
indexer_controller_set_mb_solr_url(IndexerController *self, const char *url)
{
    g_return_if_fail(INDEXER_IS_CONTROLLER(self));
    g_free(self->mb_solr_url);
...
void
indexer_controller_set_acoustid_pg_conninfo(IndexerController *self, const char *conninfo)
{
    g_return_if_fail(INDEXER_IS_CONTROLLER(self));
    g_free(self->acoustid_pg_conninfo);
...
void
indexer_controller_set_acoustid_index_url(IndexerController *self, const char *url)
{
    g_return_if_fail(INDEXER_IS_CONTROLLER(self));
    g_free(self->acoustid_index_url);
...
void
indexer_controller_set_fanart_api_key(IndexerController *self, const char *api_key)
{
    g_return_if_fail(INDEXER_IS_CONTROLLER(self));
    g_free(self->fanart_api_key);
