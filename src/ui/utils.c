...
static void
pbox_set_slot(ProportionalBox *self, GtkWidget **slot, GtkWidget *child)
{
    if (*slot)
...
static void
proportional_box_buildable_add_child(GtkBuildable *buildable,
                                     GtkBuilder *builder,
                                     GObject *child,
                                     const char *type)
{
    ...
    if (g_strcmp0(type, "art") == 0)
        pbox_set_slot(self, &self->art, w);
    else if (g_strcmp0(type, "left") == 0)
        pbox_set_slot(self, &self->col_left, w);
    else if (g_strcmp0(type, "right") == 0)
        pbox_set_slot(self, &self->col_right, w);
    else if (g_strcmp0(type, "meta") == 0)
        pbox_set_slot(self, &self->col_meta, w);
    else
        GTK_BUILDER_WARN_INVALID_CHILD_TYPE(buildable, type);
}

static void
proportional_box_buildable_init(GtkBuildableIface *iface)
{
...
static void
proportional_box_get_property(GObject *obj, guint id, GValue *val, GParamSpec *pspec)
{
    ProportionalBox *self = QUADRATURE_PROPORTIONAL_BOX(obj);
    switch (id) {
    case PROP_SPACING:
        g_value_set_int(val, self->spacing);
        break;
    case PROP_LEFT_RATIO:
        g_value_set_double(val, self->left_ratio);
        break;
    default:
...
static void
proportional_box_set_property(GObject *obj, guint id, const GValue *val, GParamSpec *pspec)
{
    ProportionalBox *self = QUADRATURE_PROPORTIONAL_BOX(obj);
    switch (id) {
    case PROP_SPACING:
...
static void
proportional_box_dispose(GObject *obj)
{
    ProportionalBox *self = QUADRATURE_PROPORTIONAL_BOX(obj);
...
static void
proportional_box_measure(GtkWidget *widget,
                         ...
                         int *nat_baseline)
{
    ...
    for (int i = 0; i < 2; i++) {
        if (!fixed_slots[i] || !gtk_widget_get_visible(fixed_slots[i]))
            continue;
        int cmin, cnat;
    ...
    for (int i = 0; i < 2; i++) {
        if (!flexible_slots[i] || !gtk_widget_get_visible(flexible_slots[i]))
            continue;
        int cmin, cnat;
...
static void
proportional_box_size_allocate(GtkWidget *widget, int width, int height, int baseline)
{
    ProportionalBox *self = QUADRATURE_PROPORTIONAL_BOX(widget);
    int sp = self->spacing;
    ...
    if (has_meta) {
        int mn, nat;
        gtk_widget_measure(
            self->col_meta, GTK_ORIENTATION_HORIZONTAL, height, &mn, &nat, NULL, NULL);
        meta_w = nat;
    }

    int n_visible
        = (has_art ? 1 : 0) + (has_left ? 1 : 0) + (has_right ? 1 : 0) + (has_meta ? 1 : 0);
    ...
    int x = 0;
    if (has_art) {
        gtk_widget_size_allocate(self->art, &(GtkAllocation){ x, 0, art_w, height }, baseline);
        x += art_w + sp;
    }
    if (has_left) {
        gtk_widget_size_allocate(
            self->col_left, &(GtkAllocation){ x, 0, left_w, height }, baseline);
        x += left_w + sp;
    }
    if (has_right) {
        gtk_widget_size_allocate(
            self->col_right, &(GtkAllocation){ x, 0, right_w, height }, baseline);
        x += right_w + sp;
    }
    if (has_meta) {
        gtk_widget_size_allocate(
            self->col_meta, &(GtkAllocation){ x, 0, meta_w, height }, baseline);
    }
}

void
proportional_box_set_pre_allocate(ProportionalBox *self,
                                  const char *slot,
...
static void
proportional_box_snapshot(GtkWidget *widget, GtkSnapshot *snapshot)
{
    ProportionalBox *self = QUADRATURE_PROPORTIONAL_BOX(widget);
    GtkWidget *slots[] = { self->art, self->col_left, self->col_right, self->col_meta };
    for (int i = 0; i < 4; i++) {
        if (slots[i])
            gtk_widget_snapshot_child(widget, slots[i], snapshot);
    }
}
...
static void
proportional_box_class_init(ProportionalBoxClass *klass)
{
    ...
    prop_specs[PROP_SPACING] = g_param_spec_int("spacing",
                                                "Spacing",
                                                "Gap between child slots in pixels",
                                                0,
                                                G_MAXINT,
                                                0,
                                                G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);

    prop_specs[PROP_LEFT_RATIO]
        = g_param_spec_double("left-ratio",
                              "Left Ratio",
...
static void
proportional_box_init(ProportionalBox *self)
{
    self->left_ratio = 0.5;
    self->spacing = 0;
}
...
static guint
ofb_child_count(GtkWidget *widget)
{
    guint n = 0;
...
static GtkWidget *
ofb_nth_child(GtkWidget *widget, guint n)
{
...
static void
quadrature_overflow_box_get_property(GObject *obj, guint id, GValue *val, GParamSpec *pspec)
{
    QuadratureOverflowBox *self = QUADRATURE_OVERFLOW_BOX(obj);
    switch (id) {
    case OFB_PROP_SPACING:
        g_value_set_int(val, self->spacing);
        break;
    case OFB_PROP_ROW_SPACING:
        g_value_set_int(val, self->row_spacing);
        break;
    case OFB_PROP_PINNED_COUNT:
        g_value_set_uint(val, self->pinned_count);
        break;
    case OFB_PROP_ITEM_COUNT:
        g_value_set_uint(val, self->item_count);
        break;
    case OFB_PROP_MAX_ROWS:
        g_value_set_uint(val, self->max_rows);
        break;
    default:
        G_OBJECT_WARN_INVALID_PROPERTY_ID(obj, id, pspec);
    }
...
static void
quadrature_overflow_box_set_property(GObject *obj, guint id, const GValue *val, GParamSpec *pspec)
{
    QuadratureOverflowBox *self = QUADRATURE_OVERFLOW_BOX(obj);
    switch (id) {
    case OFB_PROP_SPACING:
        ...
        break;
    case OFB_PROP_ROW_SPACING:
        self->row_spacing = g_value_get_int(val);
        break;
    case OFB_PROP_PINNED_COUNT:
        self->pinned_count = g_value_get_uint(val);
        break;
    case OFB_PROP_ITEM_COUNT:
        self->item_count = g_value_get_uint(val);
        break;
    case OFB_PROP_MAX_ROWS:
        self->max_rows = g_value_get_uint(val);
        break;
    default:
        G_OBJECT_WARN_INVALID_PROPERTY_ID(obj, id, pspec);
        return;
...
static void
quadrature_overflow_box_dispose(GObject *obj)
{
...
quadrature_overflow_box_get_request_mode(GtkWidget *widget)
{
    (void)widget;
...
static void
quadrature_overflow_box_measure(GtkWidget *widget,
                                GtkOrientation orientation,
                                int for_size,
                                int *minimum,
                                int *natural,
                                ...
                                int *nat_baseline)
{
    ...
    if (orientation == GTK_ORIENTATION_HORIZONTAL) {
        ...
        for (GtkWidget *c = gtk_widget_get_first_child(widget); c;
             c = gtk_widget_get_next_sibling(c)) {
            int cnat;
            gtk_widget_measure(c, GTK_ORIENTATION_HORIZONTAL, -1, NULL, &cnat, NULL, NULL);
    ...
    for (GtkWidget *c = gtk_widget_get_first_child(widget); c; c = gtk_widget_get_next_sibling(c)) {
        int cnat;
        gtk_widget_measure(c, GTK_ORIENTATION_VERTICAL, -1, NULL, &cnat, NULL, NULL);
...
static void
quadrature_overflow_box_size_allocate(GtkWidget *widget, int width, int height, int baseline)
{
    QuadratureOverflowBox *self = QUADRATURE_OVERFLOW_BOX(widget);
    ...
    GtkWidget *overflow_widget = has_overflow_child ? ofb_nth_child(widget, total - 1) : NULL;
    ...
    if (overflow_widget) {
        int h = 0;
        gtk_widget_measure(
            overflow_widget, GTK_ORIENTATION_HORIZONTAL, -1, NULL, &overflow_nat_w, NULL, NULL);
        gtk_widget_measure(overflow_widget, GTK_ORIENTATION_VERTICAL, -1, NULL, &h, NULL, NULL);
        row_h = MAX(row_h, h);
    }
    if (row_h == 0)
        row_h = height;
    ...
    if (max_rows > 0 && total_rows > max_rows) {
        ...
        while (show > pinned_count && row_of[show - 1] == last_row) {
            ...
            if (end_x + self->spacing + overflow_nat_w <= width)
                break;
            show--;
        }
    }
    ...
    guint align_rows = (max_rows > 0 && total_rows > max_rows) ? max_rows : total_rows;
    ...
    if (halign == GTK_ALIGN_END || halign == GTK_ALIGN_CENTER) {
        ...
        if (needs_overflow && overflow_widget) {
            guint ov_row = (show > 0) ? row_of[show - 1] : (max_rows > 0 ? max_rows - 1 : 0);
            if (ov_row < align_rows) {
                int ov_end_x
                    = (show > 0) ? x_of[show - 1] + nat_w[show - 1] + self->spacing + overflow_nat_w
                                 : overflow_nat_w;
                if (ov_end_x > row_end[ov_row])
                    row_end[ov_row] = ov_end_x;
            }
        }
        for (guint r = 0; r < align_rows; r++) {
    ...
    for (guint i = 0; i < flow_n && child; i++) {
        ...
        if (i < show) {
            ...
            gtk_widget_size_allocate(
                child, &(GtkAllocation){ x_of[i] + shift, y, nat_w[i], row_h }, baseline);
        } else {
            gtk_widget_set_child_visible(child, FALSE);
            gtk_widget_size_allocate(child, &(GtkAllocation){ 0, 0, nat_w[i], row_h }, baseline);
        }
        child = next;
    }
    ...
    for (; child && child != overflow_widget;) {
        GtkWidget *next = gtk_widget_get_next_sibling(child);
        int cnat = 0;
        gtk_widget_measure(child, GTK_ORIENTATION_HORIZONTAL, -1, NULL, &cnat, NULL, NULL);
        gtk_widget_set_child_visible(child, FALSE);
        gtk_widget_size_allocate(
            child, &(GtkAllocation){ 0, 0, MAX(cnat, 1), row_h > 0 ? row_h : 1 }, baseline);
        child = next;
    }
    ...
    if (overflow_widget) {
        if (needs_overflow) {
            ...
            gtk_widget_size_allocate(overflow_widget,
                                     &(GtkAllocation){ ov_x + shift, y, overflow_nat_w, row_h },
                                     baseline);
        } else {
            gtk_widget_set_child_visible(overflow_widget, FALSE);
            gtk_widget_size_allocate(
                overflow_widget,
                &(GtkAllocation){ 0, 0, overflow_nat_w, row_h > 0 ? row_h : 1 },
                baseline);
        }
    }
}
...
static void
quadrature_overflow_box_snapshot(GtkWidget *widget, GtkSnapshot *snapshot)
{
    for (GtkWidget *c = gtk_widget_get_first_child(widget); c; c = gtk_widget_get_next_sibling(c)) {
...
static void
quadrature_overflow_box_class_init(QuadratureOverflowBoxClass *klass)
{
    GObjectClass *obj_class = G_OBJECT_CLASS(klass);
    GtkWidgetClass *widget_class = GTK_WIDGET_CLASS(klass);
    ...
    ofb_props[OFB_PROP_SPACING] = g_param_spec_int("spacing",
                                                   "Spacing",
    ...
    ofb_props[OFB_PROP_ROW_SPACING] = g_param_spec_int("row-spacing",
                                                       "Row Spacing",
    ...
    ofb_props[OFB_PROP_PINNED_COUNT]
        = g_param_spec_uint("pinned-count",
                            "Pinned Count",
    ...
    ofb_props[OFB_PROP_MAX_ROWS] = g_param_spec_uint("max-rows",
                                                     "Max Rows",
                                                     "Maximum visible rows (1 = single-row)",
                                                     1,
                                                     G_MAXUINT,
                                                     1,
                                                     G_PARAM_READWRITE | G_PARAM_STATIC_STRINGS);
...
static void
quadrature_overflow_box_init(QuadratureOverflowBox *self)
{
    self->spacing = 0;
    self->row_spacing = 0;
    self->max_rows = 1;
...
void
quadrature_overflow_box_append(QuadratureOverflowBox *self, GtkWidget *child)
{
    gtk_widget_set_parent(child, GTK_WIDGET(self));
}

void
quadrature_overflow_box_set_item_count(QuadratureOverflowBox *self, guint count)
{
...
void
quadrature_overflow_box_set_pinned_count(QuadratureOverflowBox *self, guint count)
{
...
void
quadrature_overflow_box_clear_items(QuadratureOverflowBox *self)
{
...
void
quadrature_overflow_box_clear_all(QuadratureOverflowBox *self)
{
    GtkWidget *child = gtk_widget_get_first_child(GTK_WIDGET(self));
    while (child) {
        GtkWidget *next = gtk_widget_get_next_sibling(child);
...
static void
ss_step(SmoothScrollState *ss, double dt)
{
...
static gboolean
smooth_scroll_tick(GtkWidget *widget, GdkFrameClock *clock, gpointer data)
{
    (void)widget;
    ...
    if (dt_s > 0.05)
        dt_s = 0.05; /* max 50ms = 20fps floor */
    ...
    if ((clamped <= 0.0 && ss->velocity < 0.0) || (clamped >= max_val && ss->velocity > 0.0)) {
        ss->velocity = 0.0;
        ss->position = clamped;
        ss->target = clamped;
    }
...
static gboolean
smooth_scroll_event(GtkEventControllerScroll *ctrl, double dx, double dy, gpointer data)
{
    (void)ctrl;
    (void)dx;
    SmoothScrollState *ss = data;
...
static void
smooth_scroll_state_free(gpointer data)
{
    SmoothScrollState *ss = data;
...
void
ui_smooth_scroll_attach(GtkScrolledWindow *sw)
{
    ...
    GtkEventControllerScroll *ctrl = GTK_EVENT_CONTROLLER_SCROLL(gtk_event_controller_scroll_new(
    ...
    g_object_set_data_full(G_OBJECT(sw), "smooth-scroll-state", ss, smooth_scroll_state_free);
