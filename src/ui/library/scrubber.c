...
static void
cancel_animation(QuadScrubber *self)
{
...
static gboolean
scroll_tick_cb(GtkWidget *widget, GdkFrameClock *clock, gpointer data)
{
    (void)data;
    ...
    double value
        = self->anim.start_value + ease * (self->anim.target_value - self->anim.start_value);
...
void
scrubber_bucket_free(ScrubberBucket *bucket)
{
    if (!bucket)
        return;
    g_free(bucket->label);
...
static int
find_active_bucket(QuadScrubber *self, guint approx_pos)
{
    if (!self->buckets || self->buckets->len == 0)
        return -1;
...
static int
bucket_from_fraction(QuadScrubber *self, double frac)
{
...
static double
y_to_fraction(QuadScrubber *self, double y)
{
...
static void
apply_scroll(QuadScrubber *self)
{
    if (!self->vadj)
        return;
    ...
    double value
        = CLAMP(self->scroll_fraction * MAX(0.0, upper - page_size), 0.0, upper - page_size);
    gtk_adjustment_set_value(self->vadj, value);
}

/* Animate to the start of bucket at idx — used for click jumps */
static void
scroll_to_bucket_animated(QuadScrubber *self, int idx)
{
    ...
    self->anim.target_value = CLAMP(fraction * MAX(0.0, upper - page_size), 0.0, upper - page_size);
    self->anim.start_time_us = g_get_monotonic_time();
    self->anim.tick_id = gtk_widget_add_tick_callback(GTK_WIDGET(self), scroll_tick_cb, NULL, NULL);
}

static void
update_badge(QuadScrubber *self, int idx, double y)
{
    if (!self->badge)
        return;
...
static void
on_enter(GtkEventControllerMotion *ctrl, double x, double y, gpointer data)
{
    (void)ctrl;
    (void)x;
    (void)y;
...
static void
on_click_pressed(GtkGestureClick *gesture, int n_press, double x, double y, gpointer data)
{
    (void)gesture;
    (void)n_press;
    (void)x;
    QuadScrubber *self = QUAD_SCRUBBER(data);
    if (!self->buckets || self->total_items == 0)
        return;
...
{
    QuadScrubber *self = QUAD_SCRUBBER(data);
    if (self->dragging || !self->buckets || self->total_items == 0)
        return;
...
static void
quad_scrubber_snapshot(GtkWidget *widget, GtkSnapshot *snapshot)
{
    GTK_WIDGET_CLASS(quad_scrubber_parent_class)->snapshot(widget, snapshot);

    QuadScrubber *self = QUAD_SCRUBBER(widget);
    if (!self->buckets || self->buckets->len == 0 || self->total_items == 0)
        return;
    ...
    graphene_rect_t rail_rect
        = GRAPHENE_RECT_INIT(rail_x - 0.5f, (float)SCRUB_PADDING, 1.0f, usable_h);
    ...
    for (guint i = 0; i < self->buckets->len; i++) {
        ...
        if (y < prev_y + MIN_LABEL_SPACING && (int)i != self->active_idx)
            continue;
        prev_y = y;
        ...
        float tick_len
            = (float)TICK_LENGTH_MIN + ((float)TICK_LENGTH_MAX - (float)TICK_LENGTH_MIN) * bell;
        ...
        graphene_point_t label_pt = GRAPHENE_POINT_INIT(label_x, floorf(y - (float)lh / 2.0f));
        ...
        graphene_rect_t tick_rect = GRAPHENE_RECT_INIT(floorf(tick_start_x) + 0.5f,
                                                       tick_y - tick_h * 0.5f,
                                                       rail_x - floorf(tick_start_x) - 0.5f,
                                                       tick_h);
...
static void
quad_scrubber_dispose(GObject *object)
{
...
static void
quad_scrubber_class_init(QuadScrubberClass *klass)
{
...
static void
quad_scrubber_init(QuadScrubber *self)
{
    self->active_idx = -1;
...
GtkWidget *
quad_scrubber_new(void)
{
    return g_object_new(QUAD_TYPE_SCRUBBER, NULL);
}

void
quad_scrubber_set_list_view(QuadScrubber *self, GtkListView *list_view)
{
...
void
quad_scrubber_set_vadj(QuadScrubber *self, GtkAdjustment *vadj)
{
    g_assert(QUAD_IS_SCRUBBER(self));
    if (self->vadj && self->vadj_signal) {
        g_signal_handler_disconnect(self->vadj, self->vadj_signal);
    ...
    if (vadj)
        self->vadj_signal
            = g_signal_connect(vadj, "value-changed", G_CALLBACK(on_vadj_changed), self);
}

void
quad_scrubber_set_badge(QuadScrubber *self, GtkWidget *badge)
{
    g_assert(QUAD_IS_SCRUBBER(self));
    self->badge = badge;
}
...
void
quad_scrubber_set_buckets(QuadScrubber *self, GPtrArray *buckets)
{
    g_assert(QUAD_IS_SCRUBBER(self));
    if (self->buckets)
        g_ptr_array_unref(self->buckets);
...
void
quad_scrubber_set_total(QuadScrubber *self, guint total_items)
{
    g_assert(QUAD_IS_SCRUBBER(self));
...
void
quad_scrubber_clear(QuadScrubber *self)
{
    g_assert(QUAD_IS_SCRUBBER(self));
    cancel_animation(self);
    if (self->buckets) {
