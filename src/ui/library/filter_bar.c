...
static void
set_field_active(GtkWidget *menu_button, gboolean active)
{
...
static void
update_genre_label(FilterBarState *fb)
{
    if (!fb->filter_genre)
        return;
...
static void
update_year_label(FilterBarState *fb)
{
    if (!fb->filter_year)
        return;
...
static void
update_role_label(FilterBarState *fb)
{
    if (!fb->filter_role)
        return;
    ...
    if (count == 0) {
        ...
        for (int i = 0; i < NUM_ROLE_FILTERS; i++) {
            if (fb->selected_roles_mask & (1u << i)) {
                gtk_menu_button_set_label(GTK_MENU_BUTTON(fb->filter_role), ROLE_FILTERS[i].name);
                break;
            }
        }
    } else {
...
static void
sync_sort_dropdown(FilterBarState *fb)
{
    if (!fb->sort_dropdown || fb->sort_option_count == 0)
        return;
...
static void
notify_changed(FilterBarState *fb)
{
    if (fb->on_changed)
...
static void
uncheck_all_in_container(GtkWidget *widget)
{
    for (GtkWidget *child = gtk_widget_get_first_child(widget); child;
        ...
        if (GTK_IS_CHECK_BUTTON(child))
            ...
            uncheck_all_in_container(gtk_scrolled_window_get_child(GTK_SCROLLED_WINDOW(child)));
...
static void
uncheck_popover(GtkMenuButton *mb)
{
...
static void
on_genre_search_changed(GtkSearchEntry *entry, gpointer data)
{
    GtkWidget *checkbox_box = GTK_WIDGET(data);
    const char *query = gtk_editable_get_text(GTK_EDITABLE(entry));

    for (GtkWidget *child = gtk_widget_get_first_child(checkbox_box); child;
         child = gtk_widget_get_next_sibling(child)) {
        if (!GTK_IS_CHECK_BUTTON(child))
            continue;
        const char *label = gtk_check_button_get_label(GTK_CHECK_BUTTON(child));
        gboolean visible = (!query || !*query || strcasestr(label, query) != NULL);
...
static void
on_genre_popover_show(GtkWidget *popover, gpointer data)
{
    (void)popover;
...
static void
on_genre_toggled(GtkCheckButton *check, gpointer data)
{
...
static void
on_year_toggled(GtkCheckButton *check, gpointer data)
{
    FilterBarState *fb = data;
...
static void
on_role_toggled(GtkCheckButton *check, gpointer data)
{
    FilterBarState *fb = data;
...
static void
debounce_restart(guint *timer, GSourceFunc fire_cb, gpointer data)
{
    if (*timer)
...
static gboolean
on_filter_debounce_fire(gpointer data)
{
    FilterBarState *fb = data;
...
static void
on_filter_search_changed(GtkSearchEntry *entry, gpointer data)
{
    (void)entry;
    FilterBarState *fb = data;
...
static void
clear_multiselect(FilterBarState *fb)
{
    if (fb->selected_genres)
        g_hash_table_remove_all(fb->selected_genres);
...
static void
on_clear_clicked(GtkButton *btn, gpointer data)
{
    (void)btn;
    FilterBarState *fb = data;
    filter_bar_clear(fb);
...
static void
on_sort_action_activate(GSimpleAction *action, GVariant *param, gpointer data)
{
    FilterBarState *fb = data;
    const char *idx_str = g_variant_get_string(param, NULL);
    int idx = atoi(idx_str);
    if (idx < 0 || idx >= fb->sort_option_count)
        return;
    g_simple_action_set_state(action, g_variant_new_string(idx_str));
    fb->current_sort_index = idx;
    gtk_menu_button_set_label(GTK_MENU_BUTTON(fb->sort_dropdown), fb->sort_options[idx].label);
    notify_changed(fb);
}

static void
setup_sort_menu(FilterBarState *fb)
{
    if (!fb->sort_dropdown || fb->sort_option_count == 0)
        return;
    ...
    GSimpleAction *action
        = g_simple_action_new_stateful("order", G_VARIANT_TYPE_STRING, g_variant_new_string("0"));
    g_signal_connect(action, "activate", G_CALLBACK(on_sort_action_activate), fb);
    ...
    for (int i = 0; i < fb->sort_option_count; i++) {
        ...
        g_menu_item_set_action_and_target_value(item, "sort.order", g_variant_new_string(idx_str));
        g_menu_append_item(menu, item);
        g_object_unref(item);
    }

    gtk_widget_insert_action_group(
        GTK_WIDGET(fb->sort_dropdown), "sort", G_ACTION_GROUP(fb->sort_actions));
    gtk_menu_button_set_menu_model(GTK_MENU_BUTTON(fb->sort_dropdown), G_MENU_MODEL(menu));
    gtk_menu_button_set_label(GTK_MENU_BUTTON(fb->sort_dropdown), fb->sort_options[0].label);
    g_object_unref(menu);
}
...
static void
on_search_mode_activate(GSimpleAction *action, GVariant *param, gpointer data)
{
    FilterBarState *fb = data;
    const char *mode_str = g_variant_get_string(param, NULL);
    int mode = atoi(mode_str);
    if (mode < 0 || mode >= NUM_SEARCH_MODES)
        return;
    g_simple_action_set_state(action, g_variant_new_string(mode_str));
    fb->search_mode = (FilterSearchMode)mode;
    gtk_menu_button_set_label(GTK_MENU_BUTTON(fb->search_mode_dropdown), SEARCH_MODE_LABELS[mode]);
...
static void
setup_search_mode_menu(FilterBarState *fb)
{
    ...
    GSimpleAction *action
        = g_simple_action_new_stateful("mode", G_VARIANT_TYPE_STRING, g_variant_new_string("0"));
    g_signal_connect(action, "activate", G_CALLBACK(on_search_mode_activate), fb);
    ...
    for (int i = 0; i < NUM_SEARCH_MODES; i++) {
        ...
        g_menu_item_set_action_and_target_value(item, "smode.mode", g_variant_new_string(idx_str));
        g_menu_append_item(menu, item);
        g_object_unref(item);
    }

    gtk_widget_insert_action_group(
        GTK_WIDGET(fb->search_mode_dropdown), "smode", G_ACTION_GROUP(group));
    gtk_menu_button_set_menu_model(GTK_MENU_BUTTON(fb->search_mode_dropdown), G_MENU_MODEL(menu));
...
void
filter_bar_rebuild_genre_popover(FilterBarState *fb)
{
    if (!fb->filter_genre || !fb->cache)
        return;

    GPtrArray *albums = library_cache_get_albums_filtered(
        fb->cache, LIBRARY_SORT_NAME_ASC, NULL, NULL, LIBRARY_MASK_ALL);
    if (!albums)
        return;
    ...
    g_signal_connect(search, "search-changed", G_CALLBACK(on_genre_search_changed), checkbox_box);
    ...
    g_signal_connect(popover, "show", G_CALLBACK(on_genre_popover_show), search);
...
static void
build_year_popover(FilterBarState *fb)
{
    if (!fb->filter_year)
        return;
...
static void
build_role_popover(FilterBarState *fb)
{
    if (!fb->filter_role)
        return;
...
static void
on_show_featuring_toggled(GtkToggleButton *btn, gpointer data)
{
    (void)btn;
    FilterBarState *fb = data;
    notify_changed(fb);
...
GtkWidget *
filter_bar_init(FilterBarState *fb,
                library_cache_t *cache,
                const FilterBarSortOption *sort_options,
                int sort_count,
                ...
                gpointer user_data)
{
    ...
    /* Load template */
    GtkBuilder *builder = gtk_builder_new_from_resource("/org/quadrature/ui/filter_sort_bar.ui");
    ...
    fb->show_featuring_toggle
        = GTK_WIDGET(gtk_builder_get_object(builder, "show_featuring_toggle"));
    ...
    if (fb->filter_search)
        g_signal_connect(
            fb->filter_search, "search-changed", G_CALLBACK(on_filter_search_changed), fb);
    if (fb->filter_clear)
        g_signal_connect(fb->filter_clear, "clicked", G_CALLBACK(on_clear_clicked), fb);
    if (fb->show_featuring_toggle)
        g_signal_connect(
            fb->show_featuring_toggle, "toggled", G_CALLBACK(on_show_featuring_toggled), fb);

    return fb->bar_widget;
}

void
filter_bar_set_metadata_mode(FilterBarState *fb, gboolean metadata)
{
    if (fb->role_box)
        gtk_widget_set_visible(fb->role_box, metadata);
}

void
filter_bar_clear(FilterBarState *fb)
{
    ...
    if (fb->search_mode_dropdown)
        gtk_menu_button_set_label(GTK_MENU_BUTTON(fb->search_mode_dropdown), SEARCH_MODE_LABELS[0]);
...
void
filter_bar_destroy(FilterBarState *fb)
{
    if (fb->filter_debounce_timer) {
        g_source_remove(fb->filter_debounce_timer);
        fb->filter_debounce_timer = 0;
...
gboolean
filter_bar_is_active(const FilterBarState *fb)
{
...
gboolean
filter_bar_get_show_featuring(const FilterBarState *fb)
{
    if (!fb->show_featuring_toggle)
        return TRUE;
...
library_sort_t
filter_bar_get_sort(const FilterBarState *fb)
{
...
const char *
filter_bar_get_search_text(const FilterBarState *fb)
{
    if (!fb->filter_search)
        return NULL;
...
FilterSearchMode
filter_bar_get_search_mode(const FilterBarState *fb)
{
...
db_search_opts_t
filter_bar_build_search_opts(const FilterBarState *fb,
...
void
filter_bar_hide_search(FilterBarState *fb)
{
