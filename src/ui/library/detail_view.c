...
static void
unified_detail_data_free(UnifiedDetailData *ud)
{
    if (!ud)
        return;
 ...
 */
static char *
find_artist_background(app_settings_t *settings, const char *mbid)
{
    if (!settings || !mbid)
        return NULL;
...
static void
async_image_result_free(AsyncImageResult *r)
{
...
static void
load_image_thread(GTask *task, gpointer src, gpointer data, GCancellable *cancel)
{
    (void)src;
    char *path = data;
...
static void
on_bio_background_loaded(GObject *src, GAsyncResult *res, gpointer data)
{
    (void)src;
    ...
    if (ud->nav_depth > 0) {
        ...
        if (top->meta_artist_mbid)
            mbid = top->meta_artist_mbid;
    }
    if (!mbid && ud->meta_artist_mbid)
        mbid = ud->meta_artist_mbid;
    if (!mbid && ud->state == DETAIL_STATE_ARTIST && ud->current_id > 0) {
        /* MBID is universal — resolve from any library, not just the filtered set */
        const library_artist_info_t *a
            = library_cache_get_artist(ud->cache, ud->current_id, LIBRARY_MASK_ALL);
        if (a)
...
static void
load_artist_bio(UnifiedDetailData *ud, const char *mbid)
{
 ...
 */
static char *
find_artist_banner(app_settings_t *settings, const char *mbid)
{
    if (!settings || !mbid)
        return NULL;
...
static void
on_banner_loaded(GObject *src, GAsyncResult *res, gpointer data)
{
    (void)src;
    UnifiedDetailData *ud = data;
    GError *error = NULL;
    ...
    gtk_picture_set_paintable(GTK_PICTURE(ud->artist_banner), GDK_PAINTABLE(result->texture));
...
static void
load_artist_banner(UnifiedDetailData *ud, const char *mbid)
{
...
static void
on_wiki_link_clicked(GtkButton *btn, gpointer data)
{
    (void)btn;
    UnifiedDetailData *ud = data;
    if (ud->about_wiki_url) {
        ...
        gtk_uri_launcher_launch(
            launcher, GTK_IS_WINDOW(toplevel) ? GTK_WINDOW(toplevel) : NULL, NULL, NULL, NULL);
...
static void
nav_push(UnifiedDetailData *ud, NavEntryType type, int64_t id, const char *view_name)
{
...
static void
nav_clear(UnifiedDetailData *ud)
{
    for (int i = 0; i < ud->nav_depth; i++) {
        g_free(ud->nav_stack[i].view_name);
        g_free(ud->nav_stack[i].meta_artist_mbid);
...
static char *
build_current_detail_label(UnifiedDetailData *ud)
{
    if (ud->state == DETAIL_STATE_ARTIST) {
        const library_artist_info_t *artist
            = library_cache_get_artist(ud->cache, ud->current_id, ud->library_mask);
        return artist ? g_strdup_printf("Artist Detail - %s", artist->name)
                      : g_strdup("Artist Detail");
    } else if (ud->state == DETAIL_STATE_META_ARTIST) {
        return ud->meta_artist_name ? g_strdup_printf("Credits - %s", ud->meta_artist_name)
                                    : g_strdup("Credits");
    } else {
        const library_album_info_t *album
            = library_cache_get_album(ud->cache, ud->current_id, ud->library_mask);
...
static void
update_back_label(UnifiedDetailData *ud)
{
    if (ud->nav_depth > 0) {
        NavEntry *top = &ud->nav_stack[ud->nav_depth - 1];
        if (top->view_name) {
...
static void load_meta_artist_state(UnifiedDetailData *ud,
...
static void populate_library_toggles(UnifiedDetailData *ud,
                                     GtkWidget *toggles_box,
                                     const library_album_info_t *album,
...
static void
on_album_library_toggle(GtkToggleButton *btn, gpointer data)
{
    UnifiedDetailData *ud = data;
    if (!gtk_toggle_button_get_active(btn))
        return;

    int64_t target_id = (int64_t)GPOINTER_TO_SIZE(g_object_get_data(G_OBJECT(btn), "album-id"));
...
static void rebuild_artist_album_card(UnifiedDetailData *ud, GtkWidget *card, int64_t new_album_id);
...
static void
on_artist_card_library_toggle(GtkToggleButton *btn, gpointer data)
{
    UnifiedDetailData *ud = data;
    if (!gtk_toggle_button_get_active(btn))
        return;
    ...
    int64_t current_card_id
        = (int64_t)GPOINTER_TO_SIZE(g_object_get_data(G_OBJECT(card), "album-id"));
...
static void
rebuild_artist_album_card(UnifiedDetailData *ud, GtkWidget *card, int64_t new_album_id)
{
    const library_album_info_t *album
        = library_cache_get_album(ud->cache, new_album_id, ud->library_mask);
    if (!album)
        return;

    /* Build (disc_num, track_num) → new track lookup */
    GPtrArray *new_tracks
        = library_cache_get_tracks_by_album(ud->cache, new_album_id, LIBRARY_MASK_ALL);
    ...
    if (track_list) {
        for (GtkWidget *row = gtk_widget_get_first_child(track_list); row;
            ...
            const library_track_info_t *new_t
                = g_hash_table_lookup(track_map, GUINT_TO_POINTER(key));
            ...
            if (new_t) {
                ...
                g_object_set_data(
                    G_OBJECT(content), "track-id", GSIZE_TO_POINTER((gsize)new_t->track_id));
                ...
                if (info_w)
                    gtk_widget_set_sensitive(info_w, TRUE);
                if (artists_w)
                    gtk_widget_set_visible(artists_w, TRUE);
            } else {
                ...
                if (info_w)
                    gtk_widget_set_sensitive(info_w, FALSE);
                if (artists_w)
                    gtk_widget_set_visible(artists_w, FALSE);
            }
        }
    }
    ...
    if (release_info) {
        const char *type
            = (meta_release && meta_release->release_type && meta_release->release_type[0])
                  ? meta_release->release_type
                  : NULL;
        char *date_str = NULL;
    ...
    if (stats) {
        char buf[32];
        snprintf(
            buf, sizeof(buf), "%u track%s", album->track_count, album->track_count == 1 ? "" : "s");
        gtk_label_set_text(GTK_LABEL(stats), buf);
    }

    /* Update stored album-id on card */
    g_object_set_data(G_OBJECT(card), "album-id", GSIZE_TO_POINTER((gsize)new_album_id));
    ...
    if (card_toggles) {
        int64_t artist_id
            = (int64_t)GPOINTER_TO_SIZE(g_object_get_data(G_OBJECT(card_toggles), "artist-id"));
        populate_library_toggles(ud,
                                 card_toggles,
                                 album,
                                 new_album_id,
                                 G_CALLBACK(on_artist_card_library_toggle),
                                 artist_id);
    }
}
...
static void
populate_library_toggles(UnifiedDetailData *ud,
                         GtkWidget *toggles_box,
                         const library_album_info_t *album,
                         int64_t active_album_id,
                         GCallback toggle_cb,
                         int64_t artist_id)
{
    ...
    GPtrArray *versions = library_cache_get_albums(
        ud->cache, album->album_id, ud->library_mask, LIBRARY_RESULTS_ALL);
    ...
    for (guint i = 0; i < versions->len; i++) {
        ...
        g_object_set_data(G_OBJECT(btn), "album-id", GSIZE_TO_POINTER((gsize)ver->album_id));
    ...
    g_object_set_data(G_OBJECT(toggles_box), "artist-id", GSIZE_TO_POINTER((gsize)artist_id));
...
static void
load_album_state(UnifiedDetailData *ud, int64_t album_id, int64_t select_track_id)
{
    ...
    const library_album_info_t *album
        = library_cache_get_album(ud->cache, album_id, ud->library_mask);
    if (!album)
        return;
    ...
    GtkWidget *card = ui_create_album_detail_card(album,
                                                  tracks,
                                                  ud->cache,
                                                  ud->art_mgr,
                                                  0,
    ...
    if (toggles_box)
        populate_library_toggles(
            ud, toggles_box, album, album_id, G_CALLBACK(on_album_library_toggle), 0);
    ...
    if (artist_link) {
        if (ui_is_various_artists(album->artist_name)) {
            ...
            g_signal_connect(
                artist_link, "clicked", G_CALLBACK(on_unified_artist_link_clicked), ud);
        }
    }
...
static void
on_artist_library_toggle_clicked(GtkToggleButton *btn, gpointer data)
{
    UnifiedDetailData *ud = data;
    if (!gtk_toggle_button_get_active(btn))
        return;

    int64_t target_id = (int64_t)GPOINTER_TO_SIZE(g_object_get_data(G_OBJECT(btn), "artist-id"));
    if (target_id == ud->current_id)
        return;
...
static void
populate_artist_library_toggles(UnifiedDetailData *ud, int64_t active_artist_id)
{
    ...
    GPtrArray *versions = library_cache_get_artists(
        ud->cache, active_artist_id, ud->library_mask, LIBRARY_RESULTS_ALL);
    if (!versions || versions->len <= 1) {
        g_clear_pointer(&versions, g_ptr_array_unref);
        gtk_widget_set_visible(toggles_box, FALSE);
        return;
    ...
    for (guint i = 0; i < versions->len; i++) {
        ...
        g_object_set_data(G_OBJECT(btn), "artist-id", GSIZE_TO_POINTER((gsize)ver->artist_id));
...
static int
detail_album_year_desc_cmp(gconstpointer a, gconstpointer b)
{
...
static void
load_artist_state(UnifiedDetailData *ud, int64_t artist_id)
{
    ...
    GPtrArray *appearance_albums
        = library_cache_get_artist_appearances(ud->cache, artist_id, ud->library_mask);
    GPtrArray *appearance_tracks
        = library_cache_get_artist_appearance_tracks(ud->cache, artist_id, ud->library_mask);
    ...
    const library_artist_info_t *artist_check
        = library_cache_get_artist(ud->cache, artist_id, LIBRARY_MASK_ALL);
    ...
    if (has_albums) {
        ...
        const library_artist_info_t *artist
            = library_cache_get_artist(ud->cache, artist_id, ud->library_mask);
        if (artist)
    ...
    if (has_albums && ud->cache) {
        ...
        GPtrArray *first_tracks
            = library_cache_get_tracks_by_album(ud->cache, first_album->album_id, LIBRARY_MASK_ALL);
    ...
    /* Compute stats: total tracks and duration across own albums */
    {
        ...
        for (guint i = 0; i < album_count; i++) {
            ...
            GPtrArray *trks
                = library_cache_get_tracks_by_album(ud->cache, al->album_id, LIBRARY_MASK_ALL);
            if (!trks)
                continue;
    ...
    for (guint i = 0; i < album_count; i++) {
        ...
        GPtrArray *tracks
            = library_cache_get_tracks_by_album(ud->cache, album->album_id, LIBRARY_MASK_ALL);
        if (tracks && tracks->len > 0) {
            GtkWidget *card = ui_create_album_detail_card(album,
                                                          tracks,
                                                          ud->cache,
            ...
            GtkWidget *artist_link = find_widget_by_name(card, "card_artist_link");
            if (artist_link
                && (album->artist_id == artist_id || ui_is_various_artists(album->artist_name))) {
                /* Current artist or synthetic "Various Artists" - make button inactive */
                gtk_widget_set_sensitive(artist_link, FALSE);
            } else if (artist_link) {
                /* Different artist - connect navigation handler */
                g_signal_connect(
                    artist_link, "clicked", G_CALLBACK(on_album_card_artist_navigate), ud);
            }
    ...
    GHashTable *skip_album_mbids = g_hash_table_new_full(g_str_hash, g_str_equal, g_free, NULL);
    ...
    if (appearance_albums && appearance_albums->len > 0) {
        for (guint i = 0; i < appearance_albums->len; i++) {
            ...
            GPtrArray *roles
                = (credit_album_roles && album->musicbrainz_release_id)
    ...
    if (artist_info && artist_info->musicbrainz_id && ud->settings) {
        credit_count = append_credit_rows(ud,
                                          artist_info->musicbrainz_id,
                                          artist_name ? artist_name : "Unknown Artist",
                                          artist_id,
                                          skip_track_ids,
    ...
    gboolean has_appear_tracks
        = (appearance_tracks && appearance_tracks->len > 0) || credit_count > 0;
    ...
    while (album_child) {
        ...
        if (tl)
            gtk_list_box_unselect_all(GTK_LIST_BOX(tl));
    ...
    if (artist_scroll) {
        gtk_adjustment_set_value(
            gtk_scrolled_window_get_vadjustment(GTK_SCROLLED_WINDOW(artist_scroll)), 0.0);
...
static void
on_unified_back_clicked(GtkButton *btn, gpointer data)
{
    (void)btn;
    UnifiedDetailData *ud = data;
...
static void
on_unified_artist_link_clicked(GtkButton *btn, gpointer data)
{
    (void)btn;
    UnifiedDetailData *ud = data;
...
void
on_album_card_artist_navigate(GtkButton *btn, gpointer data)
{
    UnifiedDetailData *ud = data;
...
static void
on_revealer_hidden(GtkRevealer *revealer, GParamSpec *pspec, gpointer data)
{
    (void)pspec;
...
static void
on_toggle_albums(GtkToggleButton *btn, gpointer data)
{
    UnifiedDetailData *ud = data;
    if (gtk_toggle_button_get_active(btn)) {
...
static void
on_toggle_tracks(GtkToggleButton *btn, gpointer data)
{
    UnifiedDetailData *ud = data;
    if (gtk_toggle_button_get_active(btn)) {
 ...
 */
static gboolean
list_box_select_first(GtkListBox *list_box)
{
...
static GtkListBox *
artist_get_album_track_list(UnifiedDetailData *ud, int index)
{
...
static int
artist_get_selected_album_index(UnifiedDetailData *ud)
{
    GtkWidget *child = gtk_widget_get_first_child(ud->artist_albums_container);
    int i = 0;
    while (child) {
    ...
    GtkEventControllerKey *ctl, guint keyval, guint keycode, GdkModifierType state, gpointer data)
{
    ...
    if ((keyval == GDK_KEY_Down || keyval == GDK_KEY_KP_Down) && !ctrl) {
        if (ud->state == DETAIL_STATE_ALBUM) {
            ...
            if (track_list && gtk_list_box_get_selected_row(GTK_LIST_BOX(track_list)) == NULL) {
...
static void
sync_spacer_heights(UnifiedDetailData *ud)
{
    int min_h, nat_h;
    gtk_widget_measure(ud->back_header, GTK_ORIENTATION_VERTICAL, -1, &min_h, &nat_h, NULL, NULL);
    if (nat_h <= 0)
        return;
...
static void
on_back_header_height_changed(GObject *obj, GParamSpec *pspec, gpointer data)
{
    (void)obj;
    (void)pspec;
...
static GtkWidget *
build_album_page(UnifiedDetailData *ud)
{
...
static GtkWidget *
build_artist_page(UnifiedDetailData *ud)
{
    ...
    ud->artist_library_toggles
        = GTK_WIDGET(gtk_builder_get_object(builder, "artist_library_toggles"));
    ud->albums_section = GTK_WIDGET(gtk_builder_get_object(builder, "albums_section"));
    ud->artist_albums_container
        = GTK_WIDGET(gtk_builder_get_object(builder, "artist_albums_container"));
    ud->appears_on_section = GTK_WIDGET(gtk_builder_get_object(builder, "appears_on_section"));
    ud->appears_on_albums_revealer
        = GTK_WIDGET(gtk_builder_get_object(builder, "appears_on_albums_revealer"));
    ud->appears_on_tracks_revealer
        = GTK_WIDGET(gtk_builder_get_object(builder, "appears_on_tracks_revealer"));
    ...
    ud->about_background_image
        = GTK_WIDGET(gtk_builder_get_object(builder, "about_background_image"));
    ...
    ud->artist_banner_overlay
        = GTK_WIDGET(gtk_builder_get_object(builder, "artist_banner_overlay"));
    ud->artist_banner = GTK_WIDGET(gtk_builder_get_object(builder, "artist_banner"));
    ud->artist_banner_fade_bottom
        = (QuadGradientFade *)gtk_builder_get_object(builder, "artist_banner_fade_bottom");

    /* About section gradient overlays */
    ud->about_fade_top = (QuadGradientFade *)gtk_builder_get_object(builder, "about_fade_top");
    ud->about_fade_bottom
        = (QuadGradientFade *)gtk_builder_get_object(builder, "about_fade_bottom");
    ...
    g_signal_connect(
        ud->appears_on_albums, "row-activated", G_CALLBACK(ui_list_box_row_activated), NULL);
    g_signal_connect(
        ud->appears_on_tracks, "row-activated", G_CALLBACK(ui_list_box_row_activated), NULL);
    ...
    GtkWidget *artist_page_content
        = GTK_WIDGET(gtk_builder_get_object(builder, "artist_page_content"));
 ...
 */
static void
load_meta_artist_state(UnifiedDetailData *ud,
                       const char *artist_mbid,
                       const char *artist_name,
                       const char *artist_type)
{
    ...
    guint num_credits = append_credit_rows(ud,
                                           artist_mbid,
                                           artist_name ? artist_name : "Unknown Artist",
                                           0,
                                           NULL,
...
GtkWidget *
library_unified_detail_view_new(library_cache_t *cache,
                                ...
                                app_settings_t *settings)
{
    ...
    GtkBuilder *builder
        = gtk_builder_new_from_resource("/org/quadrature/ui/unified_detail_view.ui");
    ...
    sync_spacer_heights(ud);
    ud->header_height_signal = g_signal_connect(
        ud->back_header, "notify::height", G_CALLBACK(on_back_header_height_changed), ud);
    ...
    g_signal_connect(ud->appears_on_albums_revealer,
                     "notify::child-revealed",
                     G_CALLBACK(on_revealer_hidden),
                     ud);
    g_signal_connect(ud->appears_on_tracks_revealer,
                     "notify::child-revealed",
                     G_CALLBACK(on_revealer_hidden),
                     ud);
    ...
    g_object_set_data_full(
        G_OBJECT(view), UNIFIED_DATA_KEY, ud, (GDestroyNotify)unified_detail_data_free);
    gtk_stack_set_visible_child_name(GTK_STACK(ud->content_stack), "album");
...
void
library_unified_detail_navigate_to_artist(GtkWidget *view,
                                          int64_t artist_id,
...
void
library_unified_detail_navigate_to_album(GtkWidget *view,
                                         int64_t album_id,
                                         const char *source_view,
                                         int64_t select_track_id)
{
    ...
    if (select_track_id > 0) {
        ...
        g_info("Navigate → Album Detail: '%s' by %s (album_id=%" G_GINT64_FORMAT
               ") → Select track '%s' (track_id=%" G_GINT64_FORMAT ")%s%s",
               album_title,
               album_artist,
               album_id,
               track_title,
...
void
library_unified_detail_navigate_to_meta_artist(GtkWidget *view,
                                               const char *artist_mbid,
                                               const char *artist_name,
                                               const char *artist_type)
{
    ...
    if (ud->state == DETAIL_STATE_META_ARTIST && artist_mbid && ud->meta_artist_mbid
        && g_strcmp0(ud->meta_artist_mbid, artist_mbid) == 0)
        return;
...
gboolean
library_unified_detail_go_back(GtkWidget *view)
{
    UnifiedDetailData *ud = g_object_get_data(G_OBJECT(view), UNIFIED_DATA_KEY);
    g_assert(ud != NULL); /* View must have been created via library_unified_detail_view_new */
    ...
    case NAV_ENTRY_ARTIST: {
        const library_artist_info_t *artist
            = library_cache_get_artist(ud->cache, prev->id, ud->library_mask);
        const char *artist_name = artist ? artist->name : "<unknown>";
    ...
    case NAV_ENTRY_ALBUM: {
        const library_album_info_t *album
            = library_cache_get_album(ud->cache, prev->id, ud->library_mask);
        const char *album_title = album ? album->title : "<unknown>";
        const char *album_artist = album ? album->artist_name : "<unknown>";
    ...
    case NAV_ENTRY_META_ARTIST:
        ...
        load_meta_artist_state(
            ud, prev->meta_artist_mbid, prev->meta_artist_name, prev->meta_artist_type);
        break;
    }
...
void
library_unified_detail_reload(GtkWidget *view)
{
    UnifiedDetailData *ud = g_object_get_data(G_OBJECT(view), UNIFIED_DATA_KEY);
    if (!ud)
        return;
...
void
library_unified_detail_set_library_mask(GtkWidget *view, uint32_t mask)
{
    UnifiedDetailData *ud = g_object_get_data(G_OBJECT(view), UNIFIED_DATA_KEY);
    if (ud)
        ud->library_mask = mask;
}

void
library_unified_detail_clear_nav(GtkWidget *view)
{
    UnifiedDetailData *ud = g_object_get_data(G_OBJECT(view), UNIFIED_DATA_KEY);
    if (ud)
        nav_clear(ud);
...
GtkWidget *
library_unified_detail_get_track_list(GtkWidget *view)
{
    UnifiedDetailData *ud = g_object_get_data(G_OBJECT(view), UNIFIED_DATA_KEY);
    if (!ud)
        return NULL;
