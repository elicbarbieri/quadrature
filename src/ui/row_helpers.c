...
static void
on_group_row_selected(GtkListBox *list_box, GtkListBoxRow *row, gpointer data)
{
    SelectionGroup *g = data;
    if (!row || g->deselecting)
        return;
    g->deselecting = TRUE;
...
void
ui_selection_group_add(SelectionGroup *g, GtkListBox *list)
{
...
ui_selection_group_free(SelectionGroup *g)
{
    ...
    for (guint i = 0; i < g->lists->len; i++)
        g_signal_handlers_disconnect_by_func(
            g_ptr_array_index(g->lists, i), on_group_row_selected, g);
...
GtkWidget *
find_widget_by_name(GtkWidget *parent, const char *name)
{
...
static GtkWidget *
make_library_badge_label(library_cache_t *cache, int lib_idx, int max_chars)
{
...
void
ui_populate_library_badges(GtkWidget *badges_box,
                           ...
                           GtkWidget *constraint_widget)
{
    ...
    case BADGE_ENTITY_ARTIST:
        count
            = library_cache_get_artist_libraries(cache, entity_global_id, deduped, BADGE_MAX_LIBS);
        break;
    case BADGE_ENTITY_ALBUM:
        count = library_cache_get_album_libraries(cache, entity_global_id, deduped, BADGE_MAX_LIBS);
        break;
    case BADGE_ENTITY_TRACK:
        count = library_cache_get_track_libraries(cache, entity_global_id, deduped, BADGE_MAX_LIBS);
        break;
    }
...
GtkWidget *
ui_genre_pills_new(int spacing)
{
    g_type_ensure(QUADRATURE_TYPE_OVERFLOW_BOX);
    GtkWidget *box = g_object_new(QUADRATURE_TYPE_OVERFLOW_BOX, "spacing", spacing, NULL);
...
void
ui_genre_pills_bind(GtkWidget *genres_box, const char *genres)
{
    if (!genres_box)
        return;
...
static void
row_handler_data_free(gpointer data)
{
    g_free(data);
}

static int64_t
get_row_entity_id(GtkWidget *row)
{
    gpointer p;
...
static void
on_row_secondary(GtkGestureClick *gesture, int n_press, double x, double y, gpointer user_data)
{
    (void)n_press;
    (void)x;
    (void)y;
...
void
ui_row_attach_handlers(GtkWidget *row, RowCallbacks *callbacks)
{
    g_assert(row != NULL);
...
static gboolean
row_activate_idle(gpointer data)
{
...
void
ui_list_box_row_activated(GtkListBox *list, GtkListBoxRow *row, gpointer user_data)
{
    (void)list;
    (void)user_data;
    ...
    GtkEventControllerKey *ctl, guint keyval, guint keycode, GdkModifierType state, gpointer data)
{
    ...
    for (size_t i = 0; i < G_N_ELEMENTS(popover_shortcuts); i++) {
        if (keyval == popover_shortcuts[i].keyval && mods == popover_shortcuts[i].mods) {
            ...
            if (win)
                gtk_widget_activate_action_variant(win, popover_shortcuts[i].action + 4, NULL);
            return TRUE;
        }
    }
    return FALSE;
...
static void
sep_snapshot(GtkWidget *widget, GtkSnapshot *snap)
{
...
static void
quad_sep_line_init(QuadSepLine *self)
{
    (void)self;
}
static void
quad_sep_line_class_init(QuadSepLineClass *klass)
{
    GTK_WIDGET_CLASS(klass)->snapshot = sep_snapshot;
}

GtkWidget *
ui_make_section_header(const char *title)
{
...
void
ui_popover_install_shortcuts(GtkPopover *popover)
{
