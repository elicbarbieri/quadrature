...
static void histogram_draw(PerfChart* base, cairo_t* cr, int w, int h) {
    ...
    if (chart->use_adaptive) {
        ...
        for (int i = 0; i < num_bins; i++)
            if (a->bin_counts[i] > max_count) max_count = a->bin_counts[i];

        for (int i = 0; i < num_bins; i++) {
            if (a->bin_counts[i] == 0) continue;
        ...
        char stats_buf[128];
        snprintf(stats_buf, sizeof(stats_buf), "p50: %.1fms  p90: %.1fms  p99: %.1fms",
                 a->p50, a->p90, a->p99);
...
static int histogram_hit_test(PerfChart* base, double x, double y) {
    ...
    if (bucket < 0 || bucket >= num_bins) return -1;
...
static char* histogram_format_tooltip(PerfChart* base, int element) {
    ...
    if (chart->use_adaptive) {
        const perf_adaptive_hist_data_t* a = &chart->adaptive;
        if (element < 0 || element >= (int)a->num_bins) return NULL;
        double percent = a->count > 0 ?
            (double)a->bin_counts[element] / (double)a->count * 100.0 : 0.0;
        return g_strdup_printf("Range: %s %s\nCount: %u\nPercent: %.1f%%",
                               a->bin_labels[element],
                               chart->unit ? chart->unit : "",
                               a->bin_counts[element], percent);
    }
...
static inline double line_chart_val_to_y(PerfLineChart* chart, int margin_top,
                                          int chart_h, double value) {
    double t;
    if (chart->log_scale) {
...
static void line_chart_draw(PerfChart* base, cairo_t* cr, int w, int h) {
    ...
    if (chart->log_scale) {
        ...
        for (int d = 0; d <= decades; d++) {
            ...
            char buf[16];
            if (val >= 1000) snprintf(buf, sizeof(buf), "%.0fk", val / 1000);
    ...
    if (chart->sample_interval_ms > 0 && chart->last_data_time_us > 0 && x_step > 0) {
        ...
        if (sample_offset > 2.0) sample_offset = 2.0;
    ...
    /* Draw each series */
    for (int s = 0; s < chart->num_series; s++) {
        if (!chart->series_visible[s]) continue;
        ...
        cairo_move_to(cr, x_base,
            line_chart_val_to_y(chart, margin_top, chart_h, chart->series_data[s][0]));
    ...
    if (chart->sample_interval_ms > 0 && priv->hover_element >= 0) {
        if (max_count >= 2 && (size_t)priv->hover_element < max_count) {
            ...
            cairo_set_source_rgba(cr, COLOR_TEXT_LIGHT.red, COLOR_TEXT_LIGHT.green,
                                  COLOR_TEXT_LIGHT.blue, 0.5);
            cairo_set_line_width(cr, 1);
            ...
            for (int s = 0; s < chart->num_series; s++) {
                if (!chart->series_visible[s] || !chart->series_data[s]) continue;
                if ((size_t)priv->hover_element >= chart->series_count[s]) continue;
    ...
    for (int s = chart->num_series - 1; s >= 0; s--) {
        if (!chart->series_visible[s] || !chart->series_names[s]) continue;
    ...
    if (chart->show_time_axis && chart->sample_interval_ms > 0 && max_count >= 2) {
        ...
        double tick_sec;
        if      (total_sec <= 60)  tick_sec = 10;
        else if (total_sec <= 300) tick_sec = 30;
        else if (total_sec <= 600) tick_sec = 60;
        else                       tick_sec = 120;
        ...
        for (double ago = 0; ago <= total_sec + 0.01; ago += tick_sec) {
            ...
            if (lx < margin_left) lx = margin_left;
            if (lx + ext.width > w - margin_right) lx = w - margin_right - ext.width;
...
static int line_chart_hit_test(PerfChart* base, double x, double y) {
    ...
    if (idx < 0) idx = 0;
    if ((size_t)idx >= max_count) idx = (int)(max_count - 1);
    return idx;
}

static char* line_chart_format_tooltip(PerfChart* base, int element) {
    PerfLineChart* chart = PERF_LINE_CHART(base);
    if (element < 0 || chart->sample_interval_ms <= 0) return NULL;
    ...
    double seconds_ago = (double)((int)max_count - 1 - element)
                         * chart->sample_interval_ms / 1000.0;
...
void perf_line_chart_scroll_reset(PerfLineChart* chart) {
    ...
    if (chart->tick_id == 0 && chart->sample_interval_ms > 0)
        chart->tick_id = gtk_widget_add_tick_callback(
            GTK_WIDGET(chart), line_chart_tick, chart, NULL);
}
...
static void stacked_area_draw(PerfChart *base, cairo_t *cr, int w, int h) {
    ...
    for (size_t i = 0; i < max_count; i++) {
        ...
        if (stack > auto_max) auto_max = stack;
    }
    ...
    for (int i = 0; i <= 4; i++) {
        ...
        char buf[16];
        if (val >= 1024) snprintf(buf, sizeof(buf), "%.1f GB", val / 1024.0);
        else             snprintf(buf, sizeof(buf), "%.0f MB", val);
        cairo_set_source_rgb(cr, COLOR_TEXT.red, COLOR_TEXT.green, COLOR_TEXT.blue);
        cairo_move_to(cr, 5, y + 4);
        cairo_show_text(cr, buf);
    }
...
static int stacked_area_hit_test(PerfChart *base, double x, double y G_GNUC_UNUSED) {
    ...
    if (chart_w <= 0 || chart_h <= 0) return -1;
    if (x < margin_left || x > priv->width - margin_right) return -1;
    if (y < margin_top || y > margin_top + chart_h) return -1;
...
static char *stacked_area_format_tooltip(PerfChart *base, int element) {
    PerfStackedAreaChart *chart = PERF_STACKED_AREA_CHART(base);
    GString *s = g_string_new(NULL);
    double total = 0;

    for (int i = 0; i < chart->num_series; i++) {
        if (!chart->series_visible[i] || !chart->series_data[i]) continue;
        if ((size_t)element >= chart->series_count[i]) continue;
        double val = chart->series_data[i][element];
        total += val;
        if (chart->series_names[i])
            g_string_append_printf(s, "%s: %.1f %s\n",
                                   chart->series_names[i], val,
...
static void perf_stacked_area_chart_dispose(GObject *obj) {
...
static void perf_stacked_area_chart_class_init(PerfStackedAreaChartClass *klass) {
...
static void perf_stacked_area_chart_init(PerfStackedAreaChart *chart) {
    for (int i = 0; i < PERF_STACKED_MAX_SERIES; i++) {
...
void perf_stacked_area_chart_set_series(PerfStackedAreaChart *chart, int series,
                                         const char *name, const GdkRGBA *color) {
...
void perf_stacked_area_chart_set_data(PerfStackedAreaChart *chart, int series,
                                       const double *values, size_t count) {
    g_return_if_fail(PERF_IS_STACKED_AREA_CHART(chart));
    g_return_if_fail(series >= 0 && series < chart->num_series);
...
void perf_stacked_area_chart_set_hover(PerfStackedAreaChart *chart,
                                        double interval_ms, const char *unit) {
    g_return_if_fail(PERF_IS_STACKED_AREA_CHART(chart));
...
static void timeline_draw(PerfChart* base, cairo_t* cr, int w, int h) {
    ...
    int start_idx = (count > PERF_TIMELINE_MAX_EVENTS)
        ? chart->event_write : 0;
    ...
    for (int i = 0; i < total; i++) {
        ...
        if (ev->timestamp_us < start_us) continue;
        if (ev->lane < 0 || ev->lane >= chart->num_lanes) continue;
    ...
    for (int i = 0; i <= 5; i++) {
        ...
        if (mins_ago == 0) snprintf(buf, sizeof(buf), "now");
        cairo_move_to(cr, x - 8, h - 5);
        cairo_show_text(cr, buf);
    }
}

static int timeline_hit_test(PerfChart* base, double x, double y) {
    PerfTimelineChart* chart = PERF_TIMELINE_CHART(base);
    PerfChartPrivate* priv = perf_chart_get_instance_private(base);
...
static char* timeline_format_tooltip(PerfChart* base, int element) {
    PerfTimelineChart* chart = PERF_TIMELINE_CHART(base);
    if (element < 0 || element >= PERF_TIMELINE_MAX_EVENTS) return NULL;
...
static void perf_timeline_chart_finalize(GObject* obj) {
    PerfTimelineChart* chart = PERF_TIMELINE_CHART(obj);
    g_free(chart->events);
    G_OBJECT_CLASS(perf_timeline_chart_parent_class)->finalize(obj);
}

static void perf_timeline_chart_class_init(PerfTimelineChartClass* klass) {
    GObjectClass* oc = G_OBJECT_CLASS(klass);
...
static void perf_timeline_chart_init(PerfTimelineChart* chart) {
