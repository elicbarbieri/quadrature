...
static void
draw_fn(GtkDrawingArea *da, cairo_t *cr, int width, int height, gpointer user_data)
{
    ...
    for (int vi = 0; vi < num_visible; vi++) {
        ...
        for (int b = 0; b < self->num_buckets; b++)
            if (self->data[g][b] > max_count)
                max_count = self->data[g][b];
    }
    ...
    for (int b = 0; b < self->num_buckets; b++) {
        ...
        for (int vi = 0; vi < num_visible; vi++) {
            ...
            double pct_of_samples
                = (group_total[g] > 0) ? (double)count / (double)group_total[g] * 100.0 : 0.0;
            double bar_h;
            if (self->use_log_scale) {
...
static void
perf_grouped_hist_init(PerfGroupedHist *self)
{
...
static void
perf_grouped_hist_class_init(PerfGroupedHistClass *klass)
{
    (void)klass;
}
...
perf_grouped_hist_new(const char *title, const char *unit, int num_groups, int num_buckets)
{
    ...
    self->num_groups
        = (num_groups > GROUPED_HIST_MAX_GROUPS) ? GROUPED_HIST_MAX_GROUPS : num_groups;
    self->num_buckets
        = (num_buckets > GROUPED_HIST_MAX_BUCKETS) ? GROUPED_HIST_MAX_BUCKETS : num_buckets;
...
void
perf_grouped_hist_set_bucket_label(PerfGroupedHist *hist, int bucket, const char *label)
{
    g_return_if_fail(PERF_IS_GROUPED_HIST(hist));
    if (bucket < 0 || bucket >= GROUPED_HIST_MAX_BUCKETS)
        return;
    if (label) {
...
void
perf_grouped_hist_set_group_visible(PerfGroupedHist *hist, int group, gboolean visible)
{
    g_return_if_fail(PERF_IS_GROUPED_HIST(hist));
    if (group < 0 || group >= GROUPED_HIST_MAX_GROUPS)
        return;
    if (hist->group_visible[group] == visible)
        return;
...
void
perf_grouped_hist_set_log_scale(PerfGroupedHist *hist, gboolean log_scale)
{
    g_return_if_fail(PERF_IS_GROUPED_HIST(hist));
