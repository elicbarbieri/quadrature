...
static double
perf_budget_max_pct(audio_pipeline_t *pipeline, int player_id)
{
...
static uint32_t
perf_read_latency_samples(audio_pipeline_t *pipeline,
                          int player_id,
                          uint16_t *out,
...
static uint32_t
perf_read_interval_samples(audio_pipeline_t *pipeline,
                           int player_id,
                           int64_t *out,
                           uint32_t max_samples,
 ...
 */
static void
compute_percentiles_from_samples(
    uint16_t *samples, uint32_t count, double *p50, double *p90, double *p99)
{
    if (count == 0) {
        *p50 = *p90 = *p99 = 0;
        return;
...
static void
compute_log_bins(uint16_t *sorted_samples[PERF_MAX_PLAYERS],
                 ...
                 PerfGroupedHist *hist)
{
    for (int p = 0; p < PERF_MAX_PLAYERS; p++) {
        ...
        for (uint32_t i = 0; i < n; i++) {
            while (bin < LOG_NUM_BINS - 1 && sorted_samples[p][i] >= LOG_EDGES[bin + 1])
                bin++;
            buckets[bin]++;
        }
 ...
 */
static double
p90_from_us_delta(const uint64_t *cur_buckets, const uint64_t *prev_buckets)
{
    uint64_t delta[PERF_HIST_BUCKETS];
    uint64_t total = 0;
    for (int i = 0; i < PERF_HIST_BUCKETS; i++) {
        /* Saturating subtract — counters only grow */
        delta[i] = cur_buckets[i] > prev_buckets[i] ? cur_buckets[i] - prev_buckets[i] : 0;
        total += delta[i];
    }
    if (total == 0)
        return 0.0;
...
static gboolean
update_dashboard(gpointer data)
{
    ...
    for (int i = 0; i < PERF_MAX_PLAYERS; i++) {
        ...
        if (priv->pipeline) {
            ...
            if (stats.budget_overruns > 0)
                dot_class = "perf-channel-warn";
            if (stats.underrun_rate_pct > 0.0f)
                dot_class = "perf-channel-error";
        }
        if (priv->channel_health_dots[i] && dot_class != priv->prev_health_class[i]) {
            if (priv->prev_health_class[i])
                gtk_widget_remove_css_class(priv->channel_health_dots[i],
                                            priv->prev_health_class[i]);
        ...
        if (priv->pipeline) {
            bool has_err = atomic_load_explicit(&priv->pipeline->players[i].device_error,
            ...
            if (has_err != priv->prev_error_border[i]) {
                if (has_err)
                    ...
                    gtk_widget_remove_css_class(priv->channel_cells[i],
                                                "perf-channel-error-border");
    ...
    if (priv->pipeline) {
        ...
        for (int i = 0; i < nevents; i++) {
            ...
            int lane = (ev->player_id >= 0 && ev->player_id < PERF_MAX_PLAYERS) ? ev->player_id : 4;
            int severity = 1;
            const char *type_name = "Unknown";
            switch (ev->type) {
            case AUDIO_EVENT_BUFFER_UNDERRUN:
                type_name = "Buffer Underrun";
                severity = 2;
                break;
            ...
            case AUDIO_EVENT_BUDGET_OVERRUN:
                type_name = "Budget Overrun";
                severity = 1;
                break;
            case AUDIO_EVENT_INSTANT_ADVANCE:
                type_name = "Instant Advance";
                severity = 0;
                break;
            case AUDIO_EVENT_DEFERRED_ADVANCE:
                type_name = "Deferred Advance";
                severity = 2;
                break;
            ...
            case AUDIO_EVENT_BUDGET_OVERRUN:
                snprintf(tooltip,
                         ...
                         ev->data.budget.budget_ns > 0
                             ? (double)ev->data.budget.elapsed_ns
                                   / (double)ev->data.budget.budget_ns * 100.0
                             : 0.0);
                break;
            case AUDIO_EVENT_SCHEDULING_DELAY:
                snprintf(tooltip,
            ...
            perf_timeline_chart_add_event(
                priv->timeline, ev->timestamp_ns / 1000, lane, severity, type_name, tooltip);
        }
    }
    ...
    if (slow_tick && priv->dashboard && priv->mem_stacked_chart) {
        ...
        if (priv->library_cache) {
            ...
            if (lib_count > PERF_MAX_LIBRARIES)
                lib_count = PERF_MAX_LIBRARIES;
        ...
        if (priv->artwork_mgr) {
            ...
            if (art_stats.lib_count > lib_count)
                lib_count = art_stats.lib_count;
        }
    ...
    if (slow_tick && priv->artwork_mgr && priv->cache_lat_chart) {
        ...
        if (tex_hist && atlas_hist) {
            ...
            size_t n = priv->lat_write_idx < PERF_TIMESERIES_SIZE ? priv->lat_write_idx
                                                                  : PERF_TIMESERIES_SIZE;
            ...
            double ceiling = 10.0;
            if (max_p90 > 1.0)
                ceiling = pow(10.0, ceil(log10(max_p90)));
            if (ceiling < 10.0)
                ceiling = 10.0;
    ...
    if (priv->audio_cache && priv->decode_time_hist) {
        ...
        if (event_count > 0) {
            ...
            int target = 12;
            if ((int)event_count < target)
                target = (int)event_count;
            if (target < 2)
                target = 2;
            if (target > PERF_ADAPTIVE_MAX_BINS)
                target = PERF_ADAPTIVE_MAX_BINS;
            ...
            for (int b = 0; b < target; b++) {
                ...
                snprintf(adaptive.bin_labels[b], sizeof(adaptive.bin_labels[b]), "%.1f", edge);
            }
...
static void
on_reconnect_clicked(GtkButton *btn, gpointer data)
{
    (void)btn;
...
static void
on_perf_map(GtkWidget *w, gpointer data)
{
    (void)w;
    PerfViewPrivate *priv = data;
    if (priv->timer_id == 0) {
...
static void
on_perf_unmap(GtkWidget *w, gpointer data)
{
    (void)w;
    PerfViewPrivate *priv = data;
    if (priv->timer_id) {
...
static void
on_view_destroy(GtkWidget *widget, gpointer data)
{
    (void)widget;
    PerfViewPrivate *priv = data;
    if (priv->timer_id) {
...
GtkWidget *
perf_view_new(audio_pipeline_t *pipeline,
              library_cache_t *library_cache,
              ArtworkManager *artwork_mgr)
{
    ...
    for (int i = 0; i < PERF_MAX_PLAYERS; i++) {
        GtkBuilder *cell_builder
            = gtk_builder_new_from_resource("/org/quadrature/ui/perf_channel_cell.ui");
        ...
        priv->channel_reconnect_btns[i]
            = GTK_WIDGET(gtk_builder_get_object(cell_builder, "reconnect"));
        ...
        if (pipeline) {
            ...
            rd->player_id = i;
            g_signal_connect_data(priv->channel_reconnect_btns[i],
                                  "clicked",
                                  G_CALLBACK(on_reconnect_clicked),
                                  rd,
                                  (GClosureNotify)(void (*)(void))g_free,
                                  0);
        }
    ...
    GtkWidget *timeline_container
        = GTK_WIDGET(gtk_builder_get_object(builder, "timeline_container"));
    ...
    GtkWidget *latency_hist_container
        = GTK_WIDGET(gtk_builder_get_object(builder, "latency_hist_container"));
    priv->latency_hist
        = PERF_GROUPED_HIST(perf_grouped_hist_new("", "µs", PERF_MAX_PLAYERS, LOG_NUM_BINS));
    ...
    GtkWidget *mem_container = GTK_WIDGET(gtk_builder_get_object(builder, "memory_container"));
    {
        ...
        priv->mem_stacked_chart
            = PERF_STACKED_AREA_CHART(perf_stacked_area_chart_new("", total_series));
        ...
        perf_stacked_area_chart_set_series(
            priv->mem_stacked_chart, 1 + lib_count, "Art Textures", NULL);
        ...
        for (int i = 0; i < lib_count; i++) {
            ...
            perf_stacked_area_chart_set_series(
                priv->mem_stacked_chart, 2 + lib_count + i, label, NULL);
        }
    ...
    /* ── Section 5: Cache Latency log-Y line chart ── */
    GtkWidget *cache_lat_container
        = GTK_WIDGET(gtk_builder_get_object(builder, "cache_lat_container"));
    {
