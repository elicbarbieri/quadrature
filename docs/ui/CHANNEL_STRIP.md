...
┌──────────────────────────────────────────────────────────────────────┐
│  ┌─────────────────────────────────────────────────────┐ [rpt][>>]   │
│  │  Album Name (marquee)                  -3:42:00     │ [P][Q]      │
│  │  Song Title (marquee)                               │ spectrum    │
│  │  Artist     (marquee)                      3/12     │ spectrum    │
│  │  Next: Next Track Title   (marquee)                 │             │
│  └─────────────────────────────────────────────────────┘             │
...
1. **Time Remaining** (top-right) -- Large, prominent for cueing
1. **Song Title** -- Marquee scroll for long titles
1. **Artist and Track Position** -- Artist (clickable) left, "3/12" right
1. **Next Track** -- Shows upcoming track

Interactive elements: Album and Artist names brighten on hover, cursor changes to pointer. Clicking a blank area in the display panel will
toggle the focus for that channel. Only one channel can be focused at a time. Focusing makes right-clicking a track/album in the library UI queue that channel
instaed of needing to use the 1-4 hotkeys
...
1. UI updates display immediately (title, artist, album)
1. UI loads audio and sets player track
1. Audio catches up when decode completes
