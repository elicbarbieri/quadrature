...
/* Artist info (used internally by cache warming) */
typedef struct {
    int64_t id;
    char *name;
...
/**
 * Open a database.
 *   path=NULL     → in-memory (readonly ignored; always read/write)
 *   readonly=true → read-only handle (skips schema init + app-level locking;
 *                   WAL isolation handles concurrent writers)
 *   readonly=false → read/write handle with schema migrations applied
 */
...
void db_tracks_free(db_track_t *tracks, size_t count); /* count=1 for a single-track free */
...
quadrature_result_t db_iter_all_artists(quadrature_db_t *db, db_artist_iter_cb cb, void *user_data);

/** Stream all albums ordered by rowid. */
typedef bool (*db_album_iter_cb)(const db_album_t *album, void *user_data);
quadrature_result_t db_iter_all_albums(quadrature_db_t *db, db_album_iter_cb cb, void *user_data);
...
typedef bool (*db_track_artist_iter_cb)(
...
#define MB_STATUS_HAS_RELEASE_ID \
    1 /* Release UUID found in Picard tags (Phase 2); not yet fetched from MB PG */
...
/* =============================================================================
 * Indexer Error Queries (write ops in <quadrature/db_write.h>)
 * ============================================================================= */
...
quadrature_result_t db_get_album_mtimes_page(
 ...
 */
quadrature_result_t db_get_track_by_position(quadrature_db_t *db,
...
quadrature_result_t db_resolve_track_positions_batch(quadrature_db_t *db,
...
/**
 * Get total count for an entity table (artists, albums, or tracks).
 */
