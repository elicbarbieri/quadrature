/**
 * Quadrature Indexer API
 *
 * Five-phase library indexer for scanning and updating the music database:
 * - Phase 1 - SCAN: Fast directory walk, build work queue of changed albums
 * - Phase 2 - METADATA: Parallel metadata extraction (FFmpeg) + fingerprinting
 * - Phase 3 - ARTWORK: Parallel image processing (atlas)
 * - Phase 4 - RESOLVE: MusicBrainz resolution via local PG (optional)
 * - Phase 5 - FINALIZE: Batch DB updates (mtimes, error flags, checkpoint)
 *
...
typedef enum {
    ...
    INDEXER_PHASE_METADATA,      /* Parallel metadata extraction + fingerprinting */
    INDEXER_PHASE_ARTWORK,       /* Parallel artwork processing */
    INDEXER_PHASE_RESOLVE,       /* MusicBrainz resolution via local PG (optional) */
...
typedef struct {
    ...
    /* AcoustID / MusicBrainz (requires HAVE_MUSICBRAINZ) */
    bool fingerprint_tracks;  /* Generate AcoustID chromaprint fingerprints during metadata phase */
    bool mb_resolve;          /* Run MusicBrainz resolver after indexing (requires pg_conninfo) */
