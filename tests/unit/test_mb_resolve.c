...
Test(mb_resolve, acoustid_http_post) {
    ...
    cr_assert_eq(mb_pg_client_create(mb_conninfo, &mb_client), QUADRATURE_OK,
                 "failed to connect to MB PG");
    mb_pg_set_schema(mb_client, "musicbrainz");
    cr_assert_eq(mb_pg_client_create(ac_conninfo, &ac_client), QUADRATURE_OK,
    ...
    cr_assert_eq(res, QUADRATURE_OK,
                 "mb_acoustid_lookup must return OK even for no-match fingerprints");
...
Test(mb_resolve, acoustid_pg_lookup) {
    ...
    cr_assert_eq(mb_pg_client_create(conninfo, &client), QUADRATURE_OK,
                 "failed to connect to acoustid PG");

    // Step 1: get the most-submitted track_id to use as test input
    PGresult* res = (PGresult*)mb_pg_exec(client,
        "SELECT track_id FROM track_mbid ORDER BY submission_count DESC LIMIT 1",
        0, NULL);
    cr_assert_not_null(res, "NULL result from acoustid PG");
    if (PQresultStatus(res) != PGRES_TUPLES_OK) {
        PQclear(res); mb_pg_client_destroy(client);
    ...
    cr_assert_eq(PQresultStatus(res), PGRES_TUPLES_OK,
                 "track_id→mbid query failed: %s", PQresultErrorMessage(res));
    cr_assert_gt(PQntuples(res), 0,
                 "no recording MBIDs for track %s", tid_str);
...
Test(mb_resolve, mb_fetch_all_batch_test) {
    ...
    cr_assert_eq(mb_pg_install_batch_function(client), QUADRATURE_OK,
                 "failed to install batch function");
    ...
    cr_assert_eq(result, QUADRATURE_OK,
                 "mb_fetch_all_batch failed for %s", release_uuid);
    ...
    cr_assert_str_eq(release->title, expected_name,
                     "title mismatch: got '%s', expected '%s'",
                     release->title, expected_name);
...
Test(mb_resolve, full_resolve) {
    ...
    cr_assert_eq(PQresultStatus(res), PGRES_TUPLES_OK,
                 "release query failed: %s", PQresultErrorMessage(res));
    ...
    cr_assert_eq(mb_resolver_create(&resolver, db, &opts, NULL, NULL),
                 QUADRATURE_OK, "mb_resolver_create failed");
    cr_assert_eq(mb_resolver_run(resolver), QUADRATURE_OK,
                 "mb_resolver_run failed");
