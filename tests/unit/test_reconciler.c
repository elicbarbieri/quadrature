 ...
 * -------------------------------------------------------------------------- */
static int64_t build_untagged_album(quadrature_db_t *db,
    ...
    int64_t album_id = 0;
    cr_assert_eq(db_create_or_get_album_by_path(db, album_path, album_title,
                                                 artist_id, 0, &album_id),
                 QUADRATURE_OK);
...
static mb_mock_t build_mb_mock(const char *const *paths,
                                const char *const *titles,
                                size_t n, int64_t artist_id,
                                const char *artist_name,
    ...
    for (size_t i = 0; i < n; i++) {
        ...
        m.tracks[i] = (desired_track_t){
            .path = paths[i],
            .present_fields =
                DESIRED_TRACK_TITLE | DESIRED_TRACK_NUM | DESIRED_TRACK_DISC |
                DESIRED_TRACK_ARTISTS,
            .title     = titles[i],
...
static void mb_mock_free(mb_mock_t *m) {
...
Test(reconciler, bronson_untagged_tracks_get_positions_from_mb) {
    ...
    const char *filenames[] = {
        "01 - FOUNDATION (FLAC 828 kbps).flac",
        "02 - HEART ATTACK (FLAC 840 kbps).flac",
        "03 - BLINE (FLAC 871 kbps).flac",
        "04 - KNOW ME (FLAC 892 kbps).flac",
        "05 - VAULTS (FLAC 901 kbps).flac",
        "06 - TENSE (FLAC 913 kbps).flac",
        "07 - CALL OUT (FLAC 872 kbps).flac",
        "08 - CONTACT (FLAC 1043 kbps).flac",
        "09 - KEEP MOVING (FLAC 912 kbps).flac",
        "10 - DAWN (FLAC 886 kbps).flac",
    };
    ...
    int64_t album_id = build_untagged_album(
        db, "BRONSON/BRONSON (2020)", "BRONSON", "BRONSON",
        filenames, N, &artist_id);

    /* --- Sanity check: all tracks currently have track_num=0 (the bug) --- */
    {
        db_track_t *tracks = NULL;
        size_t count = 0;
        cr_assert_eq(db_get_tracks_by_album(db, album_id, &tracks, &count),
                     QUADRATURE_OK);
        cr_assert_eq(count, N);
        for (size_t i = 0; i < count; i++) {
            cr_assert_eq(tracks[i].track_num, 0,
                "Pre-reconcile track_num must be 0 (untagged)");
        }
        db_tracks_free(tracks, count);
    }

    /* --- Build desired state simulating an MB Pass 2 match --- */
    mb_mock_t mock = build_mb_mock(filenames, mb_titles, N, artist_id,
                                    "BRONSON", RECONCILE_CONFIDENCE_FUZZY);

    desired_album_state_t desired = {
        .source = RECONCILE_SOURCE_MB,
        .present_fields =
            DESIRED_ALBUM_TITLE | DESIRED_ALBUM_MB_RELEASE_ID |
    ...
    cr_assert_gt(summary.tracks_updated, 0,
        "Reconciler should have updated every track");
    cr_assert_eq(summary.track_positions_changed, (int)N,
        "All %zu track positions should have been written back", N);
    cr_assert_eq(summary.track_titles_changed, (int)N,
        "All %zu track titles should have been written", N);
    ...
    /* --- Verify via DB direct read --- */
    {
        ...
        for (size_t i = 0; i < count; i++) {
            cr_assert_eq(tracks[i].track_num, (uint16_t)(i + 1),
                "Track at position %zu should have track_num=%zu (got %u)",
                i, i + 1, tracks[i].track_num);
            cr_assert_str_eq(tracks[i].title, mb_titles[i],
                "Track %zu title should be '%s' (got '%s')",
                i, mb_titles[i], tracks[i].title);
        }
        db_tracks_free(tracks, count);
    }
    ...
    library_cache_source_t src = {
        .db_path = db_path, .music_base = "/tmp",
    ...
    GPtrArray *tracks = library_cache_get_tracks_by_album(
        cache, album->album_id, LIBRARY_MASK_ALL);
    ...
    for (size_t i = 0; i < N; i++) {
        ...
        cr_assert_eq(t->track_num, (uint16_t)(i + 1),
...
Test(reconciler, idempotent_when_already_consistent) {
    ...
    int64_t artist_id = 0;
    int64_t album_id = build_untagged_album(
        db, "test/album", "Album", "Artist", filenames, N, &artist_id);

    mb_mock_t mock = build_mb_mock(filenames, titles, N, artist_id, "Artist",
                                    RECONCILE_CONFIDENCE_EXACT);
    desired_album_state_t desired = {
        .source = RECONCILE_SOURCE_MB,
        .present_fields =
            DESIRED_ALBUM_TITLE | DESIRED_ALBUM_MB_STATUS |
...
Test(reconciler, confidence_gate_blocks_low_confidence_position_writeback) {
    ...
    int64_t artist_id = 0;
    int64_t album_id = build_untagged_album(
        db, "conf/album", "Album", "Artist", filenames, 2, &artist_id);

    /* Desired with CONFIDENCE_NONE (no match) — positions must NOT be applied */
    mb_mock_t mock = build_mb_mock(filenames, titles, 2, artist_id, "Artist",
                                    RECONCILE_CONFIDENCE_NONE);
    desired_album_state_t desired = {
        .source = RECONCILE_SOURCE_MB,
    ...
    cr_assert_eq(s.track_positions_changed, 0,
    ...
    for (size_t i = 0; i < count; i++)
        cr_assert_eq(tracks[i].track_num, 0,
            "track_num must remain 0 (confidence gate blocked writeback)");
    db_tracks_free(tracks, count);
